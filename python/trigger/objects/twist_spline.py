...
class TwistSpline(object):
    ...
    def create_t_spline(
        ...
        self.scaleGrp = cmds.group(
            name=naming.parse([name, "scale"], suffix="grp"), empty=True
        )
        # self.nonScaleGrp = cmds.group(name="nonScaleGrp_%s" % name, empty=True)
        self.nonScaleGrp = cmds.group(
            name=naming.parse([name, "nonScale"], suffix="grp"), empty=True
        )
        ...
        if mode == "equalDistance":
            ...
            for index in range(
                cuts + 2
            ):  # iterates one extra to create an additional joint for orientation
                place = root_vc + (segment_vc * index)
                # j = cmds.joint(position=place, name="jIK_%s%i" % (name, index))
                j = cmds.joint(
                    position=place, name=naming.parse([name, "IK", index], suffix="j")
                )
            ...
            for index in range(0, len(cont_distances)):
                ...
                j = cmds.joint(
                    position=place,
                    name=naming.parse([name, "IK", index], suffix="j"),
                    radius=2,
                    orientation=(0, 0, 0),
                )
                ik_joints.append(j)
        ...
        for nmb, jnt in enumerate(_duplicates):
            self.defJoints.append(
                cmds.rename(jnt, naming.parse([name, nmb], suffix="jDef"))
            )
        ...
        for index in range(len(cont_distances)):
            ...
            jnt = cmds.joint(
                position=place,
                name=naming.parse([name, "splineDriver", index], suffix="j"),
                radius=5,
                orientation=(0, 0, 0),
            )
        ...
        spline_curve = cmds.curve(
            name=naming.parse([name, "spline"], suffix="crv"),
            degree=curve_type,
            point=curve_points,
        )
        # # create spline IK
        spline_ik = cmds.ikHandle(
            solver="ikSplineSolver",
            createCurve=False,
            curve=spline_curve,
            startJoint=ik_joints[0],
            endEffector=ik_joints[len(self.defJoints) - 1],
            weight=1.0,
        )
        ...
        if twistType == "infinite":
            for pole_grp in range(0, len(self.defJoints)):
                if pole_grp < len(self.defJoints) - 1:
                    rp = cmds.ikHandle(
                        startJoint=self.defJoints[pole_grp],
                        endEffector=self.defJoints[pole_grp + 1],
                    ...
                    loc = cmds.spaceLocator(
                        name=naming.parse([name, "tSpinePole", pole_grp], suffix="loc")
                    )[0]
            ...
            cmds.connectAttr(
                "{}.worldMatrix[0]".format(cont_joints[0]),
                "{}.dWorldUpMatrix".format(spline_ik[0]),
            )
            cmds.connectAttr(
                "{}.worldMatrix[0]".format(cont_joints[-1]),
                "{}.dWorldUpMatrixEnd".format(spline_ik[0]),
            )
        ...
        for jnt in range(0, len(cont_joints)):
            ...
            cmds.parentConstraint(
                _cont_curve_name, cont_joints[jnt], maintainOffset=False
            )
            ...
            for pole_grp in range(0, len(pole_groups)):
                ...
                if pole_grp == 0:
                    cmds.orientConstraint(
                        self.contCurve_Start,
                        pole_groups[pole_grp],
                        maintainOffset=False,
                    )
                elif pole_grp == len(pole_groups) - 1:
                    cmds.orientConstraint(
                        self.contCurve_End, pole_groups[pole_grp], maintainOffset=False
                    )
                else:
                    blender = cmds.createNode(
                        "blendColors", name="tSplineX_blend_{}".format(*str(pole_grp))
                    )
                    cmds.connectAttr(
                        "{}.rotate".format(pole_groups[-1]), "{}.color1".format(blender)
                    )
                    cmds.connectAttr(
                        "{}.rotate".format(pole_groups[0]), "{}.color2".format(blender)
                    )
                    cmds.connectAttr(
                        "{}.outputR".format(blender),
                        "{}.rotateX".format(pole_groups[pole_grp]),
                    )
        ...
        cmds.addAttr(
            self.attPassCont,
            shortName="volumeFactor",
            longName="Volume_Factor",
            defaultValue=1,
            attributeType="double",
            keyable=True,
        )
        ...
        for pole_grp in range(0, len(ik_joints)):
            curve_glob_mult = cmds.createNode(
                "multiplyDivide", name="curveGlobMult_{}".format(name)
            )
            cmds.setAttr("{}.operation".format(curve_glob_mult), 2)
            bone_glob_mult = cmds.createNode(
                "multiplyDivide", name="boneGlobMult_{}".format(name)
            )

            length_mult = cmds.createNode(
                "multiplyDivide", name="length_Multiplier_{}".format(name)
            )
            cmds.setAttr("{}.operation".format(length_mult), 2)

            volume_sw = cmds.createNode("blendColors", name="volumeSw_{}".format(name))
            stretch_sw = cmds.createNode(
                "blendTwoAttr", name="stretchSw_{}".format(name)
            )

            middle_point = len(ik_joints) * 0.5
            volume_pow = cmds.createNode(
                "multiplyDivide", name="volume_Power_{}".format(name)
            )
            volume_factor = cmds.createNode(
                "multiplyDivide", name="volume_Factor_{}".format(name)
            )

            cmds.connectAttr(
                "%s.volumeFactor" % self.attPassCont, "%s.input1Y" % volume_factor
            )
            cmds.connectAttr(
                "%s.volumeFactor" % self.attPassCont, "%s.input1Z" % volume_factor
            )
            ...
            cmds.connectAttr(
                "%s.preserveVol" % self.attPassCont, "%s.blender" % volume_sw
            )
            cmds.connectAttr(
                "%s.output" % bone_glob_mult, "%s.scale" % ik_joints[pole_grp]
            )
            cmds.connectAttr(
                "%s.output" % bone_glob_mult, "%s.scale" % self.defJoints[pole_grp]
            )

        # Create endLock
        # self.endLock = cmds.spaceLocator(name="endLock_%s" % name)[0]
        self.endLock = cmds.spaceLocator(
            name=naming.parse([name, "endLock"], suffix="loc")
        )[0]
        cmds.pointConstraint(
            self.defJoints[len(self.defJoints) - 1], self.endLock, maintainOffset=False
        )
        ...
        for idx in range(len(self.contCurves_ORE)):
            if idx == 0:
                functions.align_to_alter(
                    self.contCurves_ORE[idx], guide_joints[idx], mode=2
                )
            else:
                functions.align_to_alter(
                    self.contCurves_ORE[idx], guide_joints[idx], mode=0
                )
                functions.align_to_alter(
                    self.contCurves_ORE[idx], guide_joints[idx - 1], mode=1
                )
        ...
        # RETURN
        self.noTouchData = (
            [spline_curve, spline_ik[0], self.endLock],
            ik_joints,
            cont_joints,
            pole_groups,
            rp_handles,

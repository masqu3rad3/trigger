...
class TwistSpline(object):
    ...
    def create_t_spline(self, guide_joints, name, cuts=10, dropoff=2, mode="equalDistance", twistType="regular",
        ...
        dead_bone = cmds.listRelatives(ik_joints[len(ik_joints) - 1], c=True)
        ...
        for jnt in range(0, len(cont_joints)):
            ...
            functions.align_to_alter(cont_curve, cont_joints[jnt], mode=2)
            cont_curve_ore = functions.create_offset_group(cont_curve, "ORE")
            cmds.parentConstraint(cont_curve, cont_joints[jnt], maintainOffset=False)
            cont_curves.append(cont_curve)
        ...
        if twistType == "infinite":
            ...
            for pole_grp in range(0, len(pole_groups)):
                ...
                if pole_grp == 0:
                    ...
                    blender = cmds.createNode("blendColors", name="tSplineX_blend" + str(pole_grp))
                    cmds.connectAttr("%s.rotate" % pole_groups[-1], "%s.color1" % blender)
                    cmds.connectAttr("%s.rotate" % pole_groups[0], "%s.color2" % blender)
                    cmds.connectAttr("%s.outputR" % blender, "%s.rotateX" % pole_groups[pole_grp])
                    blend_ratio = (pole_grp + 0.0) / (cuts - 1.0)
                    cmds.setAttr("%s.blender" % blender, blend_ratio)
        else:
            pass
        ...
        for pole_grp in range(0, len(ik_joints)):
            curve_glob_mult = cmds.createNode("multiplyDivide", name="curveGlobMult_" + name)
            cmds.setAttr("{}.operation".format(curve_glob_mult), 2)
            bone_glob_mult = cmds.createNode("multiplyDivide", name="boneGlobMult_" + name)

            length_mult = cmds.createNode("multiplyDivide", name="length_Multiplier_" + name)
            cmds.setAttr("{}.operation".format(length_mult), 2)

            volume_sw = cmds.createNode("blendColors", name="volumeSw_" + name)
            stretch_sw = cmds.createNode("blendTwoAttr", name="stretchSw_" + name)

            middle_point = (len(ik_joints) / 2)
            volume_pow = cmds.createNode("multiplyDivide", name="volume_Power_" + name)
            volume_factor = cmds.createNode("multiplyDivide", name="volume_Factor_" + name)

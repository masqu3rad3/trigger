...
WINDOWNAME = "Rom Randomizer v0.1.0"
...
class MainUI(QtWidgets.QDialog):
    ...
    def build_ui(self):
        ...
        self.method_combobox.addItems(["Random Poses", "Random Combinations"])
    ...
    def _get_collector(self):
        ...
        if self.hook_node_select_layout.selection:
            collector.add_morph_controllers(self.hook_node_select_layout.selection[0])
        if self.additional_controllers_select_layout.selection:
            for controller in self.additional_controllers_select_layout.selection:
                collector.add_controller(controller)
    ...
    def on_clear_animation(self):
        ...
        rom = rom_randomizer.FaceRomGenerator(self._get_collector())
        rom.clear_keys()
    ...
    def on_generate_rom(self):
        ...
        seed = self.seed_spinbox.value() if self.seed_sp.isEnabled() else None
        rom.symmetry = symmetry
        if method == 0:
            rom.generate_random_poses_rom(start_frame=start, duration=end-start, interval=interval, seed=seed)
        elif method == 1:
            rom.generate_random_combo_rom(start_frame=start, duration=end-start, interval=interval, minimum_combinations=min_combinations, maximum_combinations=max_combinations, seed=seed)
        else:

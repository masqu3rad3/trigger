...
def create_type():
    ...
    cmds.connectAttr(
        "{}.outputMesh".format(type_node), "{}.inputMesh".format(type_extrude)
    )
    cmds.connectAttr(
        "{}.vertsPerChar".format(type_node), "{}.vertsPerChar".format(type_extrude)
    )
...
class RomRigger(object):
    def __init__(self):
        ...
        self.trackers = []
        self.script_job_as_string = self._import_file_as_string(
            script_job.__file__.replace("pyc", "py")
        )
...
class Tracker(object):
    ...
    def __init__(self, index):
        ...
        cmds.setAttr(
            "{}.pythonExpression".format(self.type_node), _expression, type="string"
        )

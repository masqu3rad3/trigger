...
def transfer_guides(source_mesh, target_mesh, guides_grp):
    ...
    temp_blendshape = cmds.blendShape(
        source_mesh, target_mesh, w=[0, 1], name="trTMP_guideTransfer_blendshape"
    )
    follicles = parentToSurface.parentToSurface(
        objects=all_joints, surface=target_mesh, mode="parent"
    )

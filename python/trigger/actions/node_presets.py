...
class Node_presets(ActionCore):
    ...
    def action(self):
        ...
        for node_data in data_list:
            ...
            if not os.path.exists(node_preset) and not self.skip_non_existing:
                log.error(
                    "The node preset cannot be found => %s" % node_preset, proceed=False
                )
            else:
    ...
    def ui(self, ctrl, layout, handler, *args, **kwargs):
        ...
        nodes_listbox = custom_widgets.ListBoxLayout(
            alignment="start", buttonUp=False, buttonNew=False, buttonDown=False
        )
    ...
    def load_preset(node, source_folder):
        ...
        if not os.path.isfile(source_file):
            log.error(
                "The source file doesn't exist => %s" % source_file, proceed=False
            )
        presets_folder = os.path.join(cmds.about(preferences=True), "presets")

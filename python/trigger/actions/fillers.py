...
class Fillers(ActionCore):
    ...
    def action(self):
        ...
        methods = {0: "object", 1: "shader", 2: "triswitch"}
        primary_channel = self.primary_channel or "Auto"
        for controller in self.controllers:
            filler = controller_filler.Filler(
    ...
    def ui(self, ctrl, layout, handler, *args, **kwargs):
        ...
        controllers_listbox = custom_widgets.ListBoxLayout(
            alignment="start",
            buttonNew=False,
            buttonRename=False,
            buttonUp=False,
        ...
        scaling_lbl.setToolTip(
            "If checked, the fillers will have scaling animation driven with controllers"
        )
        ...
        normalize_scale_lbl.setToolTip(
            "If checked, there wont be any negative scaling and '0' will be also 0 scale "
        ...
        color_method_combo.setToolTip(
            "Object => Simple object coloring without shaders\n"
        ...
        primary_channel_le.setToolTip(
            "The channel which will drive filler animations. Leave empty for 'auto'"
        )
        ...
        visibility_controller_lbl.setToolTip(
            "Attribute to control visibilities of fillers"
        )
        ...
        id_tag_lbl.setToolTip(
            "Common nodes (Groups, Shaders, etc) will share the same nodes with the same id tags."
        )
        ...
        coloring_cb.stateChanged.connect(
            lambda x: color_a_pb.setDisabled(x)
            if match_colors_cb.isChecked() and x
            else color_a_pb.setDisabled(not x)
        )
        coloring_cb.stateChanged.connect(
            lambda x: color_b_pb.setDisabled(x)
            if match_colors_cb.isChecked() and x
            else color_b_pb.setDisabled(not x)
        )

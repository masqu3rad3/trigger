...
def _get_plug_ids(source_deformer, source_influence=None):
    """This method is the api version of the below out-commented method for faster evolution"""
    ...
    mesh = cmds.listConnections(source_deformer, scn=True, d=True, s=False, type="mesh")[0]
    vtx_count = cmds.polyEvaluate(mesh, vertex=True)
    return plug, vtx_count


def set_deformer_weights(target_deformer, list_of_weights, target_influence, data_type="double"):
    plug, vtx_count = _get_plug_ids(target_deformer, target_influence)

    if data_type == "double":
        map(lambda i: plug.elementByLogicalIndex(i).setDouble(list_of_weights[i]), range(vtx_count))
    elif data_type == "float":
        map(lambda i: plug.elementByLogicalIndex(i).setFloat(list_of_weights[i]), range(vtx_count))
    return list_of_weights


def get_deformer_weights(source_deformer, source_influence=None, data_type="double"):
    plug, vtx_count = _get_plug_ids(source_deformer, source_influence)
    # reading
    if data_type == "double":
        weight_list = [plug.elementByLogicalIndex(i).asDouble() for i in range(vtx_count)]
    elif data_type == "float":
        weight_list = [plug.elementByLogicalIndex(i).asFloat() for i in range(vtx_count)]

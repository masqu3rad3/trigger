...
def replace_controller(
    ...
    cmds.setAttr(
        "%s.overrideEnabled" % functions.get_shapes(new_cont_dup)[0],
        cmds.getAttr("%s.overrideEnabled" % functions.get_shapes(old_controller)[0]),
    )

    cmds.setAttr(
        "%s.overrideColor" % functions.get_shapes(new_cont_dup)[0],
    ...
    if mirror:
        ...
        # get the same color code
        cmds.setAttr(
            "%s.overrideEnabled" % functions.get_shapes(new_cont_dup_mirror)[0],
            cmds.getAttr("%s.overrideEnabled")
            % functions.get_shapes(old_cont_mirror)[0],
        )
        cmds.setAttr(
            "%s.overrideColor" % functions.get_shapes(new_cont_dup_mirror)[0],
...
def replace_curve(orig_curve, new_curve, snap=True, transfer_color=True):
    ...
    if (
        cmds.objectType(orig_curve) == "transform"
        or cmds.objectType(orig_curve) == "joint"
    ):
    ...
    if (
        cmds.objectType(new_curve) == "transform"
        or cmds.objectType(new_curve) == "joint"
    ):
    ...
    for new_shape, orig_shape in zip(new_shapes, orig_shapes):
        ...
        cmds.connectAttr(
            "{}.worldSpace".format(new_shape), "{}.create".format(orig_shape)
        )
        ...
        for i in range(0, spans + degree):
            cmds.xform(
                orig_shape + ".cv[" + str(i) + "]",
                t=cmds.pointPosition(new_shape + ".cv[" + str(i) + "]"),
                ws=1,
            )
...
def mirror_controller(
    ...
    for node in node_list:
        ...
        tmp_cont = cmds.duplicate(
            node, name="tmp_{0}".format(node), rr=True, renameChildren=True
        )
...
def whip(
    ...
    cmds.addAttr(
        attr_holder, ln="delay", at="long", defaultValue=maximum_offset, k=False
    )
    ...
    cmds.addAttr(
        ...
        min=0,
        max=1,
        defaultValue=diminish,
        k=True,
    )

    for attr in attr_list:
        cmds.addAttr(
            attr_holder, at="float", ln="offsetMult_%s" % attr, defaultValue=1, k=True
        )
    ...
    for nmb, node in enumerate(node_list[1:]):
        for attr in attr_list:
            ...
            cmds.connectAttr(
                "%s.%s" % (attr_holder, "offsetMult_%s" % attr),
                "%s.input2" % master_mult,
            )
...
def copy_controller(a, b=None, axis=None, side_flags=("L_", "R_"), side_bias="start"):
    ...
    temp_cont = cmds.duplicate(
        a, name="tmp_{0}".format(a), rr=True, renameChildren=True
    )[0]
    # delete nodes below it
...
def motion_path_spline(curve_obj, num_of_objects, object_type="joint", aim=False):
    ...
    for i in range(num_of_objects):
        ...
        motion_path = cmds.shadingNode(
            "motionPath", n="%s_motionPath" % obj, asUtility=True
        )
        cmds.connectAttr(
            "%s.worldSpace[0]" % curve_obj, "%s.geometryPath" % motion_path
        )
        cmds.connectAttr(
            "%s.allCoordinates.xCoordinate" % motion_path, "%s.translateX" % obj
        )
        cmds.connectAttr(
            "%s.allCoordinates.yCoordinate" % motion_path, "%s.translateY" % obj
        )
        cmds.connectAttr(
            "%s.allCoordinates.zCoordinate" % motion_path, "%s.translateZ" % obj
        )
    ...
    if aim:
        ...
        for i in range(0, spans + degree):
            ...
            cluster, _clusterHandle = cmds.cluster(
                "{0}.cv[{1}]".format(curve_obj, str(i)),
...
def make_stretchy_ik(
    ...
    attribute.validate_attr(
        "%s.squash" % end_controller,
        attr_type="double",
        attr_range=[0.0, 1.0],
        default_value=0.0,
    )
    attribute.validate_attr(
        "%s.stretch" % end_controller,
        attr_type="double",
        attr_range=[0.0, 1.0],
        default_value=1.0,
    )
    attribute.validate_attr(
        "%s.stretchLimit" % end_controller,
        attr_type="double",
        attr_range=[0.0, 99999.0],
        default_value=100.0,
    )
    attribute.validate_attr(
        "%s.softIK" % end_controller,
        attr_type="double",
        attr_range=[0.0, 100.0],
        default_value=0.0,
    )

    # root_loc = cmds.spaceLocator(name="rootLoc_%s" % name)[0]
    root_loc = cmds.spaceLocator(
        name=naming.parse([name, "stretchy", "root"], suffix="loc")
    )[0]
    functions.align_to(root_loc, joint_chain[0], position=True, rotation=True)
    connection.matrixConstraint(
        root_controller, root_loc, skipRotate="xyz", maintainOffset=True
    )
    ...
    end_loc = cmds.spaceLocator(
        name=naming.parse([name, "stretchy", "end"], suffix="loc")
    )[0]
    ...
    soft_blend_loc = cmds.spaceLocator(
        name=naming.parse([name, "stretchy", "softBlend"], suffix="loc")
    )[0]
    ...
    if not distance_start:
        ...
        distance_start_loc = cmds.spaceLocator(
            name=naming.parse([name, "stretchy", "distanceStart"], suffix="loc")
        )[0]
        connection.matrixConstraint(
            root_controller,
            distance_start_loc,
    ...
    if not distance_end:
        ...
        distance_end_loc = cmds.spaceLocator(
            name=naming.parse([name, "stretchy", "distanceEnd"], suffix="loc")
        )[0]
        connection.matrixConstraint(
            end_controller,
            distance_end_loc,
    ...
    ctrl_distance = cmds.createNode(
        "distanceBetween", name=naming.parse([name, "stretchy"], suffix="distance")
    )
    ...
    for nmb, jnt in enumerate(joint_chain[1:]):
        ...
        cmds.addAttr(
            jnt, longName="initialDistance", attributeType="double", defaultValue=dist
        )
    ...
    cmds.connectAttr(
        "%s.worldMatrix[0]" % root_controller, "%s.matrixIn[0]" % scale_mult_matrix
    )
    cmds.connectAttr(
        "%s.matrixSum" % scale_mult_matrix, "%s.inputMatrix" % scale_decompose_matrix
    )

    global_scale_div_p = op.divide(
        1, "%s.outputScaleX" % scale_decompose_matrix, name="global_scale_div"
    )
    if is_local:
        global_mult_p = ctrl_distance_p
    else:
        global_mult_p = op.multiply(
            ctrl_distance_p, global_scale_div_p, name="global_mult"
        )
    ...
    soft_distance = cmds.createNode(
        "distanceBetween",
        name=naming.parse([name, "stretchy", "soft"], suffix="distance"),
    )
    cmds.connectAttr("%s.worldPosition[0]" % end_loc_shape, "%s.point1" % soft_distance)
    cmds.connectAttr(
        "%s.worldPosition[0]" % soft_blend_loc_shape, "%s.point2" % soft_distance
    )
    soft_distance_p = "%s.distance" % soft_distance

    stretch_global_div_p = op.divide(
        soft_distance_p,
        "%s.outputScaleX" % scale_decompose_matrix,
    ...
    for jnt in joint_chain[1:]:
        ...
        squash_blend_node = cmds.createNode(
            "blendColors",
            name=naming.parse([name, "stretchy", "squash"], suffix="blend"),
        )
        ...
        clamp_node = cmds.createNode(
            "clamp", name=naming.parse([name, "stretchy", "limit"], suffix="clamp")
        )
        max_distance_p = op.add(
            "%s.stretchLimit" % end_controller, "%s.initialDistance" % jnt
        )

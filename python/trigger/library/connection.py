...
def connections(node, exclude_nodes=None, exclude_types=None, return_mode="all"):
    ...
    raw_inputs = cmds.listConnections(
        node, plugs=True, source=True, destination=False, connections=True
    )
    raw_outputs = cmds.listConnections(
        node, plugs=True, source=False, destination=True, connections=True
    )
    ...
    if exclude_nodes:
        input_plugs = [
            plug for plug in input_plugs if plug.split(".")[0] not in exclude_nodes
        ]
    if exclude_types:
        input_plugs = [
            plug
            for plug in input_plugs
            if cmds.objectType(plug.split(".")[0]) not in exclude_types
        ]

    for in_plug in input_plugs:
        conn = {
            "plug_out": cmds.listConnections(
                in_plug, plugs=True, source=True, destination=False, connections=True
            )[0],
            "plug_in": cmds.listConnections(
                in_plug, plugs=True, source=True, destination=False, connections=True
            )[1],
        }
    ...
    for out_plug in output_plugs:
        out_connections = cmds.listConnections(
            out_plug, plugs=True, source=False, destination=True, connections=False
        )
        for out_c in out_connections:
            ...
            if exclude_types:
                if cmds.objectType(out_c.split(".")[0]) in exclude_types:
                    continue
            conn = {"plug_out": out_plug, "plug_in": out_c}
    ...
    if return_mode == "all":
        ...
        raise Exception(
            "Not valid return_mode argument."
...
def replace_connections(
    ...
    all_connections = connections(
        source_node, exclude_nodes=exclude_nodes, exclude_types=exclude_types
    )
    ...
    for con_dict in in_connections:
        ...
        existing_connections = (
            cmds.listConnections(out_p, p=True, source=False, destination=True) or []
        )
    ...
    for con_dict in out_connections:
        ...
        # stupid warnings...
        existing_connections = (
            cmds.listConnections(in_p, p=True, source=True, destination=False) or []
        )
...
def matrixConstraint(
    ...
    **short_arguments
    ...
    if is_multi and source_parent_cutoff:
        LOG.warning(
            "source_parent_cutoff is not supported for multiple inputs. Ignoring it."
        )
        source_parent_cutoff = None
    ...
    cmds.connectAttr(
        "{}.matrixSum".format(mult_matrix), "{}.inputMatrix".format(decompose_matrix)
    )

    if source_parent_cutoff:
        next_index += 1
        cmds.connectAttr(
            "{}.worldInverseMatrix".format(source_parent_cutoff),
            "{0}.matrixIn[{1}]".format(mult_matrix, next_index),
        )
    ...
    if parent_of_driven:
        next_index += 1
        cmds.connectAttr(
            "{}.worldInverseMatrix[0]".format(parent_of_driven),
            "{0}.matrixIn[{1}]".format(mult_matrix, next_index),
        )

    if not skipTranslate:
        cmds.connectAttr(
            "{}.outputTranslate".format(decompose_matrix), "{}.translate".format(driven)
        )
    else:
        for attr in "XYZ":
            if attr.lower() not in skipTranslate and attr.upper() not in skipTranslate:
                cmds.connectAttr(
                    "{0}.outputTranslate{1}".format(decompose_matrix, attr),
    ...
    if not skipRotate or len(skipRotate) != 3:
        if is_joint:
            ...
            cmds.setAttr("{}.inputRotate".format(rotation_compose), *joint_orientation)
            cmds.connectAttr(
                "{}.outputMatrix".format(rotation_compose),
                "{0}.matrixIn[{1}]".format(rotation_first_mult_matrix, rot_index),
            )

            if parent_of_driven:
                rot_index += 1
                cmds.connectAttr(
                    "{}.worldMatrix[0]".format(parent_of_driven),
                    "{0}.matrixIn[{1}]".format(rotation_first_mult_matrix, rot_index),
                )
            cmds.connectAttr(
                "{}.matrixSum".format(rotation_first_mult_matrix),
            ...
            second_index += 1
            cmds.connectAttr(
                "{}.outputMatrix".format(rotation_inverse_matrix),
                "{0}.matrixIn[{1}]".format(rotation_sec_mult_matrix, second_index),
            )
            cmds.connectAttr(
                "{}.matrixSum".format(rotation_sec_mult_matrix),
        ...
        if not skipRotate:
            ...
            for attr in "XYZ":
                if attr.lower() not in skipRotate and attr.upper() not in skipRotate:
                    cmds.connectAttr(
                        "{0}{1}".format(rotation_output_plug, attr),
                        "{0}.rotate{1}".format(driven, attr),
                    )

    if not skipScale:
        cmds.connectAttr(
            "{}.outputScale".format(decompose_matrix), "{}.scale".format(driven)
        )
    else:
        for attr in "XYZ":
            if attr.lower() not in skipScale and attr.upper() not in skipScale:
                cmds.connectAttr(
                    "{0}.outputScale{1}".format(decompose_matrix, attr),
...
def matrix_switch(
    ...
    mult_matrix_a, dump, _ = matrixConstraint(
        parent_a,
        child,
    ...
    cmds.delete(dump)
    mult_matrix_b, dump, _ = matrixConstraint(
        parent_b,
        child,
        maintainOffset=False,
        prefix=parent_b,
    ...
    wt_add_matrix = cmds.createNode(
        "wtAddMatrix", name="{0}_{1}_wtAdd".format(parent_a, parent_b)
    )
    cmds.connectAttr(
        "{}.matrixSum".format(mult_matrix_a),
        "{}.wtMatrix[0].matrixIn".format(wt_add_matrix),
    )
    cmds.connectAttr(
        "{}.matrixSum".format(mult_matrix_b),
    ...
    if source_parent_cutoff:
        mult_matrix_cutoff = cmds.createNode(
            "multMatrix", name="{0}_{1}_multMatrixCutoff".format(parent_a, parent_b)
        )
        cmds.connectAttr(
            "{}.matrixSum".format(wt_add_matrix),
    ...
    decompose_node = cmds.createNode(
        "decomposeMatrix", name="{0}_{1}_decompose_switch".format(parent_a, parent_b)
    )
    cmds.connectAttr(out_plug, "%s.inputMatrix" % decompose_node)

    if position:
        cmds.connectAttr(
            "{}.outputTranslate".format(decompose_node), "{}.translate".format(child)
        )
    if rotation:
        cmds.connectAttr(
            "{}.outputRotate".format(decompose_node), "{}.rotate".format(child)
        )
    if scale:
        cmds.connectAttr(
            "{}.outputScale".format(decompose_node), "{}.scale".format(child)
        )
...
def get_closest_uv(source_node, dest_node):
    ...
    cmds.connectAttr(
        "{}.outMesh".format(source_node), "{}.inMesh".format(closest_point_node), f=1
    )
    cmds.setAttr("{}.inPositionX".format(closest_point_node), x_pos)
    cmds.setAttr("{}.inPositionY".format(closest_point_node), y_pos)
    cmds.setAttr("{}.inPositionZ".format(closest_point_node), z_pos)
...
def create_follicle(name, surface, uv):
    ...
    cmds.connectAttr(
        "{}.worldMatrix[0]".format(surface), "{}.inputWorldMatrix".format(follicle)
    )
    cmds.connectAttr("{0}.{1}".format(surface, out), "{0}.{1}".format(follicle, inp))
    cmds.connectAttr(
        "{}.outTranslate".format(follicle), "{}.translate".format(follicle_transform)
    )
    cmds.connectAttr(
        "{}.outRotate".format(follicle), "{}.rotate".format(follicle_transform)
    )
...
def uv_pin(mesh_transform, coordinates):
    ...
    if not intermediates:
        ...
        for connection in incoming_connections:
            ...
            cmds.connectAttr(
                connection["plug_in"],
                connection["plug_out"].replace(deformed_mesh, original_mesh),
            )
...
def average_constraint(
    ...
    for i, vertex_nmb in enumerate(vertex_list):
        ...
        if force_follicle:
            ...
            cmds.connectAttr(
                "{}.worldMatrix[0]".format(pin_node),
                "{0}.wtMatrix[{1}].matrixIn".format(average_node, i),
            )
        else:
            pin_node = uv_pin(target_mesh, uv)
            cmds.connectAttr(
                "{}.outputMatrix[0]".format(pin_node),
                "{0}.wtMatrix[{1}].matrixIn".format(average_node, i),
            )
    ...
    if not offset_parent:
        ...
        cmds.connectAttr(
            "{}.matrixSum".format(average_node), "{}.matrixIn[0]".format(mult_matrix)
        )
        cmds.connectAttr(
            "{}.matrixSum".format(mult_matrix),
            "{}.inputMatrix".format(decompose_matrix),
        )
        cmds.connectAttr(
            "{}.outputTranslate".format(decompose_matrix),
            "{}.translate".format(source_object),
        )
        cmds.connectAttr(
            "{}.outputRotate".format(decompose_matrix),
        ...
        cmds.connectAttr(
            "{}.matrixSum".format(average_node), "{}.inputMatrix".format(pick_matrix)
        )
        ...
        cmds.connectAttr(
            "{}.outputMatrix".format(pick_matrix),
...
def connect_mirror(node1, node2, mirror_axis="X"):
    ...
    cmds.connectAttr("{}.translate".format(node1), "{}.input".format(rvs_node_t))
    cmds.connectAttr("{}.output".format(rvs_node_t), "{}.input3D[0]".format(minus_op_t))
    ...
    cmds.connectAttr("{}.rotate".format(node1), "{}.input".format(rvs_node_r))
    cmds.connectAttr("{}.output".format(rvs_node_r), "{}.input3D[0]".format(minus_op_r))
    ...
    if mirror_axis == "X":
        cmds.connectAttr("{}.output3Dx".format(minus_op_t), "{}.tx".format(node2))
        cmds.connectAttr("{}.ty".format(node1), "{}.ty".format(node2))
        cmds.connectAttr("{}.tz".format(node1), "{}.tz".format(node2))
        cmds.connectAttr("{}.rx".format(node1), "{}.rx".format(node2))
        cmds.connectAttr("{}.output3Dy".format(minus_op_r), "{}.ry".format(node2))
        cmds.connectAttr("{}.output3Dz".format(minus_op_r), "{}.rz".format(node2))

    if mirror_axis == "Y":
        cmds.connectAttr("{}.tx".format(node1), "{}.tx".format(node2))
        cmds.connectAttr("{}.output3Dy".format(minus_op_t), "{}.ty".format(node2))
        cmds.connectAttr("{}.tz".format(node1), "{}.tz".format(node2))
        cmds.connectAttr("{}.output3Dx".format(minus_op_r), "{}.rx".format(node2))
        cmds.connectAttr("{}.ry".format(node1), "{}.ry".format(node2))
        cmds.connectAttr("{}.output3Dz".format(minus_op_r), "{}.rz".format(node2))

    if mirror_axis == "Z":
        cmds.connectAttr("{}.tx".format(node1), "{}.tx".format(node2))
        cmds.connectAttr("{}.ty".format(node1), "{}.ty".format(node2))
        cmds.connectAttr("{}.output3Dz".format(minus_op_t), "{}.tz".format(node2))
        cmds.connectAttr("{}.rx".format(node1), "{}.rx".format(node2))
        cmds.connectAttr("{}.output3Dy".format(minus_op_r), "{}.ry".format(node2))
        cmds.connectAttr("{}.output3Dz".format(minus_op_r), "{}.rz".format(node2))
...
def matrix_constrain_localised(
    ...
    if not cmds.objExists(no_inherit_matrix):
        ...
        no_inherit_matrix = cmds.createNode("fourByFourMatrix", n=no_inherit_matrix)
        element_list = [
            "in00",
            "in01",
            "in02",
        ...
        for index, element in enumerate(element_list):
            cmds.setAttr(
                "{0}.{1}".format(no_inherit_matrix, element), grabbed_matrix[index]
            )

    # multiply all of the matrices together #
    mult_matrix = cmds.createNode("multMatrix", n="{}_multMatrix".format(inherit))
    cmds.connectAttr(
        "{}.worldMatrix[0]".format(inherit), "{}.matrixIn[0]".format(mult_matrix)
    )
    cmds.connectAttr(
        "{}.worldInverseMatrix[0]".format(no_inherit),
        "{}.matrixIn[1]".format(mult_matrix),
    )
    cmds.connectAttr(
        "{}.output".format(no_inherit_matrix), "{}.matrixIn[2]".format(mult_matrix)
    )
    # decompose result #
    decomp_matrix = cmds.createNode(
        "decomposeMatrix", name="{}_decomposeMatrix".format(destination)
    )
    cmds.connectAttr(
        "{}.matrixSum".format(mult_matrix), "{}.inputMatrix".format(decomp_matrix)
    )

    # connect up #
    for attr in target_attrs:
        output_attr = "output{0}{1}".format(
            attr[0].capitalize(), attr[1:]
        )  # capitalize only first letter

        lock_state = cmds.getAttr("{0}.{1}".format(destination, attr), lock=True)
        # unlock, connect, then reinstate lock state #
        cmds.setAttr("{0}.{1}".format(destination, attr), lock=False)  # unlock #
        cmds.connectAttr(
            "{0}.{1}".format(decomp_matrix, output_attr),

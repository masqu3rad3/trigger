...
class Finger(ModuleCore):
    ...
    def __init__(self, build_data=None, inits=None):
        ...
        self.module_name = naming.unique_name(
            "%s_%s" % (cmds.getAttr("%s.moduleName" % self.fingerRoot), self.fingerType)
        )
    ...
    def create_joints(self):
        ...
        for nmb, guide in enumerate(self.inits):
            ...
            jnt = cmds.joint(
                name=naming.parse([self.module_name, nmb], suffix=_suffix), radius=1.0
            )
    ...
    def create_controllers(self):
        ...
        for index in range(0, len(self.deformerJoints)):
            ...
            cmds.parentConstraint(
                cont.name, self.deformerJoints[index], maintainOffset=True
            )
        ...
        attribute.drive_attrs(
            "%s.contVis" % self.scaleGrp, ["%s.v" % x[0] for x in conts_off]
        )
        ...
        if self.handController:
            if cmds.objExists(self.handController):
                ...
                self.handController = Controller(
                    self.handController, shape="Square", side=self.side
                )
    ...
    def hand_setup(self):
        ...
        cmds.addAttr(
            self.handController.name,
            shortName=spread_attr,
            defaultValue=0.0,
            attributeType="float",
            keyable=True,
        )
        ...
        cmds.connectAttr(
            "{0}.{1}".format(self.handController.name, spread_attr),
            "{0}.input2".format(spread_mult),
        )
        cmds.connectAttr(
            "{}.output".format(spread_mult), "{0}.rotateY".format(self.contConList[1])
        )
        ...
        for nmb in range(0, (len(self.inits) - 1)):
            ...
            cmds.connectAttr(
                "{0}.{1}".format(self.handController.name, bend_attr),
                "%s.rotateZ" % self.contConList[nmb],
            )
...
class Guides(GuidesCore):
    ...
    def draw_joints(self):
        ...
        for seg in range(self.segments + 1):
            finger_jnt = cmds.joint(
                position=(r_point_finger + (add_finger * seg)),

...
class Fkik(_module.ModuleCore):
    ...
    def create_joints(self):
        ...
        if self.switchMode == 0:
            ...
            attribute.drive_attrs(
                "%s.rigVis" % self.scaleGrp, ["%s.v" % x for x in self.ikJoints]
            )
            attribute.drive_attrs(
                "%s.rigVis" % self.scaleGrp, ["%s.v" % x for x in self.fkJoints]
            )
    ...
    def create_controllers(self):
        ...
        if self.switchMode == 0 or self.switchMode == 1:
            ...
            for nmb, jnt in enumerate(fk_joints):
                ...
                if nmb != 0:
                    cmds.parent(
                        self.fkControllersOff[nmb], self.fkControllers[nmb - 1].name
                    )
            cmds.parent(self.fkControllersOff[0], self.localOffGrp)

            attribute.drive_attrs(
                "%s.contVis" % self.scaleGrp,
                ["%s.v" % x for x in self.fkControllersOff],
            )
        ...
        if self.switchMode == 0 or self.switchMode == 2:
            ...
            ik_bind_grp = cmds.group(
                name=naming.parse([self.module_name, "IK", "bind"], suffix="grp"),
                empty=True,
            )
            ...
            functions.align_to(
                root_ik_cont_off, ik_joints[0], rotation=True, position=True
            )
            ...
            functions.align_to(
                end_ik_cont_off, ik_joints[-1], rotation=True, position=True
            )
            ...
            if self.ikSolver != 0:  # if it is a rotate plane or spring solver
                ...
                self.polevector_bridge = cmds.spaceLocator(
                ...
                offset_vector = api.get_between_vector(
                    ik_joints[self.middleIndex], ik_joints
                )
        ...
        if self.switchMode == 0:
            scale_mult = (
                functions.get_distance(self.ikJoints[0], self.ikJoints[1]) * 0.5
            )
    ...
    def create_ik_setup(self):
        ...
        ik_handle = cmds.ikHandle(
            ...
            name=naming.parse([self.module_name], suffix="IKHandle"),
            solver=solver,
        )[0]
        ...
        if self.ikSolver != 0:
            ...
            cmds.connectAttr(
                "%s.rigVis" % self.scaleGrp, "%s.v" % self.polevector_bridge
            )

        # scaling
        _ = [
            cmds.connectAttr("%s.s" % ik_joints[0], "%s.s" % jnt)
            for jnt in ik_joints[1:]
        ]

        if self.stretchyIk:
            stretch_locs = make_stretchy_ik(
                ik_joints,
                ik_handle,
                self.rootIkCont.name,
                self.endIKCont.name,
                side=self.side,
                source_parent_cutoff=self.localOffGrp,
            ...
            attribute.drive_attrs(
                "%s.rigVis" % self.scaleGrp, ["%s.v" % x for x in stretch_locs]
            )
        else:
            connection.matrixConstraint(
                self.ikControllers[-1].name,
                ik_handle,
                maintainOffset=True,
                source_parent_cutoff=self.localOffGrp,
            )
        connection.matrixConstraint(
            self.ikControllers[0].name,
            ik_joints[0],
            maintainOffset=False,
            source_parent_cutoff=self.localOffGrp,
        )
    ...
    def create_fk_setup(self):
        ...
        for cont, jnt in zip(self.fkControllers, fk_joints):
            connection.matrixConstraint(
                cont.name, jnt, source_parent_cutoff=self.localOffGrp, skipScale="xyz"
            )
            if not self.isLocal:
                ...
                s_global = cmds.createNode(
                    "multiplyDivide",
                    name=naming.parse(
                        [self.module_name, "sGlobal", jnt], suffix="mult"
                    ),
                )
    ...
    def ik_fk_switching(self):
        ...
        for fk_jnt, ik_jnt, def_jnt in zip(
            self.fkJoints, self.ikJoints, self.deformerJoints
        ):
            blend_t = cmds.createNode(
                "blendColors",
                name=naming.parse([self.module_name, "translate"], suffix="blend"),
            )
            blend_r = cmds.createNode(
                "blendColors",
                name=naming.parse([self.module_name, "rotate"], suffix="blend"),
            )
            blend_s = cmds.createNode(
                "blendColors",
                name=naming.parse([self.module_name, "scale"], suffix="blend"),
            )
            ...
            cmds.connectAttr(
                "%s.fk_ik_reverse" % self.switch_controller.name, "%s.blender" % blend_t
            )
            cmds.connectAttr(
                "%s.fk_ik_reverse" % self.switch_controller.name, "%s.blender" % blend_r
            )
            cmds.connectAttr(
                "%s.fk_ik_reverse" % self.switch_controller.name, "%s.blender" % blend_s
            )

        for ik_co in self.ikControllers:
            cmds.connectAttr(
                "%s.fk_ik" % self.switch_controller.name, "%s.v" % ik_co.name
            )
        if self.ikSolver != 0:
            cmds.connectAttr(
                "%s.fk_ik" % self.switch_controller.name,
                "%s.v" % self.polevector_cont.name,
            )
        for fk_co in self.fkControllers:
            cmds.connectAttr(
                "%s.fk_ik_reverse" % self.switch_controller.name, "%s.v" % fk_co.name
            )
...
class Guides(_module.GuidesCore):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.segments = kwargs.get(
            "segments", 2
        )  # minimum segments required for the fk/ik module is two
    ...
    def draw_joints(self):
        ...
        for seg in range(self.segments + 1):
            ...
            jnt = cmds.joint(
                position=(r_point_tail + (seperation_value * seg) + zig_zag_offset),

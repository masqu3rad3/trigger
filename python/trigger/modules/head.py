...
class Head(_module.ModuleCore):
    def __init__(self, build_data=None, inits=None):
        ...
        # initialize suffix
        self.module_name = (naming.unique_name(cmds.getAttr("%s.moduleName" % self.neckNodes[0])))
    ...
    def create_joints(self):
        ...
        self.limbPlug = cmds.joint(name=naming.parse([self.module_name, "plug"], suffix="j"), position=self.root_pos,
                                   radius=3)

        # Create temporaray Guide Joints
        cmds.select(clear=True)
        self.guideJoints = [
            cmds.joint(name=naming.parse([self.module_name, i], suffix="jTemp"), position=api.get_world_translation(i))
            for i in self.neckNodes]
        self.guideJoints.append(
            cmds.joint(name=naming.parse([self.module_name, "head"], suffix="jTemp"), position=self.headPivPos))
        self.guideJoints.append(
            cmds.joint(name=naming.parse([self.module_name, "headEnd"], suffix="jTemp"), position=self.headEndPivPos))

        # orientations
        if not self.useRefOrientation:
            joint.orient_joints(self.guideJoints, world_up_axis=self.look_axis, up_axis=(0, 1, 0),
    ...
    def create_controllers(self):
        ...
        self.cont_neck = Controller(name=naming.parse([self.module_name, "neck"], suffix="cont"),
                                    shape="CurvedCircle",
                                    scale=neck_scale,
                                    normal=(1, 0, 0),
                                    side=self.side,
        ...
        # Head Controller
        self.cont_head = Controller(name=naming.parse([self.module_name, "head"], suffix="cont"),
                                    shape="HalfDome",
        ...
        if self.stretchyHead:
            ...
            self.cont_headSquash = Controller(name=naming.parse([self.module_name, "headSquash"], suffix="cont"),
                                              shape="Circle",
    ...
    def create_roots(self):
        self.neckRootLoc = cmds.spaceLocator(name=naming.parse([self.module_name, "neckRoot"], suffix="loc"))[0]
    ...
    def create_ik_setup(self):
        ...
        neck_spline.create_t_spline(list(self.guideJoints[:-1]), "neckSplineIK_%s" % self.module_name, self.resolution,
                                    dropoff=self.dropoff, mode=self.splineMode, twistType=self.twistType)
        self.sockets.extend(neck_spline.defJoints[:-1])  # do not add the last neck spline joint to the socket list

        # # Connect neck start to the neck controller
        cmds.orientConstraint(self.cont_neck.name, neck_spline.contCurve_Start, maintainOffset=False)
        cmds.pointConstraint(neck_spline.contCurve_Start, self.cont_neck_ORE, maintainOffset=False)
        # # Connect neck end to the head controller
        cmds.parentConstraint(self.cont_head.name, neck_spline.contCurve_End, maintainOffset=True)
        ...
        # bring out contents.
        attribute.attribute_pass(neck_spline.scaleGrp, self.scaleGrp, attributes=["sx", "sy", "sz"],
                                 keepSourceAttributes=True)
        cmds.disconnectAttr(cmds.listConnections(neck_spline.scaleGrp, plugs=True)[0],
        ...
        if self.stretchyHead:
            ...
            head_spline.create_t_spline(list(self.guideJoints[-2:]), "headSquashSplineIK_%s" % self.module_name, 3,
                                        dropoff=2, mode=self.splineMode, twistType=self.twistType)
            ...
            cmds.pointConstraint(neck_spline.endLock, head_spline.contCurve_Start, maintainOffset=False)

            # # orient the head spline to the head controller
            # TODO // FIX HERE
            cmds.orientConstraint(self.cont_head.name, head_spline.contCurve_Start, maintainOffset=True)

            functions.align_to_alter(self.cont_headSquash.name, head_spline.contCurve_End, mode=2)
            # TODO // FIX HERE
            cmds.parentConstraint(self.cont_headSquash.name, head_spline.contCurve_End, maintainOffset=True)
            attribute.attribute_pass(head_spline.attPassCont, self.cont_headSquash.name)

            # # Connect the scale to the scaleGrp
            cmds.connectAttr("%s.scale" % self.scaleGrp, "%s.scale" % head_spline.scaleGrp)
            # bring out contents.
            attribute.attribute_pass(head_spline.scaleGrp, self.scaleGrp, attributes=["sx", "sy", "sz"],
                                     keepSourceAttributes=True)
            cmds.disconnectAttr(cmds.listConnections(head_spline.scaleGrp, plugs=True)[0],
                                "%s.scale" % head_spline.scaleGrp)
            self.deformerJoints.extend(head_spline.defJoints)
        else:
            head_joint = cmds.joint(name=naming.parse([self.module_name, "head"], suffix="jDef"),
                                    position=self.headPivPos, radius=3)
            head_joint_end = cmds.joint(name=naming.parse([self.module_name, "headEnd"], suffix="jDef"),
                                        position=self.headEndPivPos, radius=3)
        ...
        for m in range(0, len(neck_spline.contCurves_ORE)):
            if 0 < m < len(neck_spline.contCurves_ORE):
                ...
                o_con = \
                    cmds.parentConstraint(self.cont_head.name, self.cont_neck.name, neck_spline.contCurves_ORE[m],
                                          maintainOffset=True)[
                        0]
                blend_ratio = (m + 0.0) / len(neck_spline.contCurves_ORE)
                cmds.setAttr("{0}.{1}W0".format(o_con, self.cont_head.name), blend_ratio)
                cmds.setAttr("{0}.{1}W1".format(o_con, self.cont_neck.name), 1 - blend_ratio)
        ...
        try:
            cmds.parent(neck_spline.contCurves_ORE[len(neck_spline.contCurves_ORE) - 1], self.scaleGrp)
        except RuntimeError:
            pass
        ...
        if self.stretchyHead:
            ...
            try:
                cmds.parent(head_spline.contCurves_ORE[len(head_spline.contCurves_ORE) - 1], self.scaleGrp)
            except RuntimeError:
                pass
        ...
        attribute.drive_attrs("%s.contVis" % self.scaleGrp, ["%s.v" % x for x in mid_controls])
        attribute.drive_attrs("%s.jointVis" % self.scaleGrp, ["%s.v" % x for x in self.deformerJoints])

        if self.stretchyHead:
            cmds.connectAttr("%s.rigVis" % self.scaleGrp, "%s.v" % head_spline.contCurves_ORE[0], force=True)
            cmds.connectAttr("%s.rigVis" % self.scaleGrp, "%s.v" % head_spline.contCurves_ORE[-1], force=True)
        cmds.connectAttr("%s.rigVis" % self.scaleGrp, "%s.v" % neck_spline.contCurves_ORE[0], force=True)
        cmds.connectAttr("%s.rigVis" % self.scaleGrp, "%s.v" % neck_spline.contCurves_ORE[-1], force=True)
...
class Guides(_module.GuidesCore):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.segments = kwargs.get("segments", 1)  # minimum segments required for the module
    ...
    def draw_joints(self):
        ...
        for seg in range(self.segments):
            neck_jnt = cmds.joint(position=(r_point_neck + (add_neck * seg)),
                                  name=naming.parse([self.name, "neck", seg], side=self.side, suffix="jInit"))
            self.guideJoints.append(neck_jnt)
        for seg in range(1):
            head_jnt = cmds.joint(position=(r_point_neck + (add_neck * (seg + self.segments))),
                                  name=naming.parse([self.name, "head", seg], side=self.side, suffix="jInit"))
            self.guideJoints.append(head_jnt)
        headEnd = cmds.joint(position=point_head,
                             name=naming.parse([self.name, "headEnd"], side=self.side, suffix="jInit"))
        self.guideJoints.append(headEnd)

        # Update the guideJoints list
        joint.orient_joints(self.guideJoints, world_up_axis=-self.lookVector, reverse_aim=self.sideMultiplier,

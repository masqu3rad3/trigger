...
class Hindleg(ModuleCore):
    ...
    def __init__(self, build_data=None, inits=None):
        ...
        # get distances
        self.init_upper_leg_dist = functions.get_distance(
            self.hindhip_ref, self.stifle_ref
        )
        self.init_lower_leg_dist = functions.get_distance(
            self.stifle_ref, self.hock_ref
        )
        self.init_pastern_dist = functions.get_distance(
            self.hock_ref, self.phalanges_ref
        )
        self.init_foot_dist = functions.get_distance(
            self.phalanges_ref, self.phalangestip_ref
        )
        ...
        self.module_name = naming.unique_name(
            cmds.getAttr("%s.moduleName" % self.hindleg_root_ref)
        )
    ...
    def create_joints(self):
        ...
        self.limbPlug = cmds.joint(
            ...
            position=self.hindleg_root_pos,
            radius=3,
        )
        ...
        cmds.select(deselect=True)
        self.j_def_hindleg_root = cmds.joint(
            name=naming.parse([self.module_name, "hindLegRoot"], suffix="jDef"),
            position=self.hindleg_root_pos,
            radius=1.5,
        )
        self.sockets.append(self.j_def_hindleg_root)
        self.deformerJoints.append(self.j_def_hindleg_root)
        self.j_def_hindhip = cmds.joint(
            name=naming.parse([self.module_name, "hindHip"], suffix="jDef"),
            position=self.hindhip_pos,
            radius=1.5,
        )
        ...
        if not self.useRefOrientation:
            joint.orient_joints(
                [self.j_def_hindleg_root, self.j_def_hindhip],
            ...
            functions.align_to(
                self.j_def_hindhip, self.hindhip_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_def_hindhip, apply=True)

        cmds.select(deselect=True)
        self.j_def_stifle = cmds.joint(
            name=naming.parse([self.module_name, "stifle"], suffix="jDef"),
            position=self.stifle_pos,
            radius=1.5,
        )
        ...
        cmds.select(deselect=True)
        self.j_def_hock = cmds.joint(
            name=naming.parse([self.module_name, "hock"], suffix="jDef"),
            position=self.hock_pos,
            radius=1.0,
        )
        ...
        cmds.select(deselect=True)
        self.j_def_phalanges = cmds.joint(
            name=naming.parse([self.module_name, "phalanges"], suffix="jDef"),
            position=self.phalanges_pos,
            radius=1.0,
        )
        ...
        cmds.select(deselect=True)
        self.j_phalanges_tip = cmds.joint(
            name=naming.parse([self.module_name, "phalangesTip"], suffix="j"),
            position=self.phalangestip_pos,
            radius=1.0,
        )
        ...
        # IK Chain
        cmds.select(deselect=True)
        self.j_ik_hip = cmds.joint(
            name=naming.parse([self.module_name, "IK", "hindHip"], suffix="j"),
            position=self.hindhip_pos,
            radius=0.5,
        )
        self.j_ik_stifle = cmds.joint(
            name=naming.parse([self.module_name, "IK", "stifle"], suffix="j"),
            position=self.stifle_pos,
            radius=0.5,
        )
        self.j_ik_hock = cmds.joint(
            name=naming.parse([self.module_name, "IK", "hock"], suffix="j"),
            position=self.hock_pos,
            radius=0.5,
        )
        self.j_ik_phalanges = cmds.joint(
            name=naming.parse([self.module_name, "IK", "phalanges"], suffix="j"),
            position=self.phalanges_pos,
            radius=0.5,
        )
        self.j_ik_phalanges_tip = cmds.joint(
            name=naming.parse([self.module_name, "IK", "phalangesTip"], suffix="j"),
            position=self.phalangestip_pos,
            radius=0.5,
        )
        cmds.select(deselect=True)
        ...
        if not self.useRefOrientation:
            ...
            functions.align_to(
                self.j_ik_hip, self.hindhip_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_ik_hip, apply=True)

            functions.align_to(
                self.j_ik_stifle, self.stifle_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_ik_stifle, apply=True)

            functions.align_to(
                self.j_ik_hock, self.hock_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_ik_hock, apply=True)

            functions.align_to(
                self.j_ik_phalanges, self.phalanges_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_ik_phalanges, apply=True)

            functions.align_to(
                self.j_ik_phalanges_tip,
                self.phalangestip_ref,
                position=True,
                rotation=True,
            )
        ...
        # FK Joints
        cmds.select(deselect=True)
        self.j_fk_hip = cmds.joint(
            name=naming.parse([self.module_name, "FK", "hindHip"], suffix="j"),
            position=self.hindhip_pos,
            radius=2.0,
        )
        self.j_fk_stifle = cmds.joint(
            name=naming.parse([self.module_name, "FK", "stifle"], suffix="j"),
            position=self.stifle_pos,
            radius=2.0,
        )
        self.j_fk_hock = cmds.joint(
            name=naming.parse([self.module_name, "FK", "hock"], suffix="j"),
            position=self.hock_pos,
            radius=2.0,
        )
        self.j_fk_phalanges = cmds.joint(
            name=naming.parse([self.module_name, "FK", "phalanges"], suffix="j"),
            position=self.phalanges_pos,
            radius=2.0,
        )
        self.j_fk_phalanges_tip = cmds.joint(
            name=naming.parse([self.module_name, "FK", "phalangesTip"], suffix="j"),
            position=self.phalangestip_pos,
            radius=2.0,
        )
        ...
        if not self.useRefOrientation:
            ...
            functions.align_to(
                self.j_fk_hip, self.hindhip_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_fk_hip, apply=True)

            functions.align_to(
                self.j_fk_stifle, self.stifle_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_fk_stifle, apply=True)

            functions.align_to(
                self.j_fk_hock, self.hock_ref, position=True, rotation=True
            )
            cmds.makeIdentity(self.j_fk_hock, apply=True)

            functions.align_to(
                self.j_fk_phalanges, self.phalanges_ref, position=True, rotation=True
            )
        ...
        connection.matrixConstraint(
            self.j_def_hindhip, self.rigJointsGrp, maintainOffset=False
        )
    ...
    def create_controllers(self):
        ...
        functions.align_to(
            self.thigh_cont.name, self.j_def_hindhip, position=True, rotation=False
        )
        functions.align_to(
            self.thigh_cont.name, self.j_def_hindleg_root, position=False, rotation=True
        )

        cmds.move(
            self.sideMult * (thigh_cont_scale[0] * 3),
            0,
            0,
        ...
        functions.align_between(
            self.foot_ik_cont.name, self.j_def_phalanges, self.j_phalanges_tip
        )
        functions.align_to(
            self.foot_ik_cont.name, self.j_def_phalanges, position=False, rotation=True
        )
        ...
        self.hock_ik_cont = Controller(
            shape="DualCurvedArrow",
        ...
        offset_vector_pole = api.get_between_vector(
            self.j_def_hock, [self.j_def_stifle, self.j_def_phalanges]
        )

        functions.align_and_aim(
            self.hock_ik_cont.name,
        ...
        functions.align_to(
            self.hock_ik_cont.name, self.j_def_phalanges, position=False, rotation=True
        )
        ...
        offset_vector_pole = api.get_between_vector(
            self.j_def_stifle, [self.j_def_hindhip, self.j_def_hock]
        )
        ...
        functions.align_to(
            self.pole_ik_cont.name, self.pole_bridge, position=True, rotation=True
        )
        ...
        self.upper_leg_fk_cont = self._create_fk_cont(
            self.j_fk_hip, self.init_upper_leg_dist, name="UpperLeg"
        )
        self.lower_leg_fk_cont = self._create_fk_cont(
            self.j_fk_stifle, self.init_lower_leg_dist, name="LowerLeg"
        )
        self.pastern_fk_cont = self._create_fk_cont(
            self.j_fk_hock, self.init_pastern_dist, name="Pastern"
        )
        self.foot_fk_cont = self._create_fk_cont(
            self.j_fk_phalanges, self.init_foot_dist, name="Foot"
        )
        ...
        cmds.setAttr(
            "{0}.fingerControls".format(self.switch_cont.name), channelBox=True
        )
    ...
    def _create_fk_cont(self, jnt, length, name=""):
        ...
        cmds.xform(
            cont.name, pivots=(self.sideMult * -(length * 0.5), 0, 0), worldSpace=True
        )
    ...
    def common(self):
        ...
        attribute.disconnect_attr(
            node=self.j_def_hindleg_root, attr="inverseScale", suppress_warnings=True
        )
    ...
    def create_ik_setup(self):
        # create ik chains
        hock_ik_handle = cmds.ikHandle(
            startJoint=self.j_ik_hip,
            endEffector=self.j_ik_hock,
            name=naming.parse([self.module_name, "hock"], suffix="IKHandle"),
            solver="ikRPsolver",
        )[0]
        phalanges_ik_handle = cmds.ikHandle(
            startJoint=self.j_ik_hock,
            endEffector=self.j_ik_phalanges,
            name=naming.parse([self.module_name, "phalanges"], suffix="IKHandle"),
            solver="ikSCsolver",
        )[0]
        phalanges_tip_ik_handle = cmds.ikHandle(
            startJoint=self.j_ik_phalanges,
            endEffector=self.j_ik_phalanges_tip,
            name=naming.parse([self.module_name, "phalangesTip"], suffix="IKHandle"),
            solver="ikSCsolver",
        )[0]

        connection.matrixConstraint(
            self.pole_ik_cont.name,
            self.pole_bridge,
        ...
        def group_align(nodes, pivot_node, name):
            loc = cmds.spaceLocator(
                name=naming.parse([self.module_name, name, "trans"], suffix="loc")
            )[0]
        ...
        hock_ik_grp, hock_ik_loc = group_align(
            hock_ik_handle, self.j_def_phalanges, "hock"
        )
        phalanges_ik_grp, phalanges_ik_loc = group_align(
            phalanges_ik_handle, self.j_def_phalanges, "phalanges"
        )
        phalanges_tip_ik_grp, phalanges_tip_ik_loc = group_align(
            phalanges_tip_ik_handle, self.j_def_phalanges, "phalangesTip"
        )
        toe_trans_grp, toe_trans_loc = group_align(
            [hock_ik_grp, phalanges_ik_grp, phalanges_tip_ik_grp],
            self.j_phalanges_tip,
            "toe",
        )
        foot_trans_grp, foot_trans_loc = group_align(
            toe_trans_grp, self.j_def_phalanges, "foot"
        )
        ...
        # stretchyness
        if self.isStretchy:
            hock_distance_start = cmds.spaceLocator(
                name=naming.parse(
                    [self.module_name, "hock", "distanceStart"], suffix="loc"
                )
            )[0]
            cmds.parent(hock_distance_start, self.nonScaleGrp)
            cmds.pointConstraint(
                self.j_def_hindhip, hock_distance_start, maintainOffset=False
            )

            hock_distance_end = cmds.spaceLocator(
                name=naming.parse(
                    [self.module_name, "hock", "distanceEnd"], suffix="loc"
                )
            )[0]
            cmds.parent(hock_distance_end, self.nonScaleGrp)
            functions.align_to(hock_distance_end, self.j_def_hock, position=True)
            connection.matrixConstraint(
                hock_ik_loc, hock_distance_end, maintainOffset=True
            )

            # create a dummy controller to be used instead of the end controller
            dummy_hock_cont = cmds.spaceLocator(
                name=naming.parse([self.module_name, "hock", "dummy"], suffix="loc")
            )[0]
            functions.align_to(
                dummy_hock_cont, hock_distance_end, position=True, rotation=True
            )
            connection.matrixConstraint(
                self.hock_ik_cont.name, dummy_hock_cont, maintainOffset=True
            )

            cmds.parent(dummy_hock_cont, self.nonScaleGrp)

            hock_stretch_locs = tools.make_stretchy_ik(
                [self.j_ik_hip, self.j_ik_stifle, self.j_ik_hock],
                hock_ik_handle,
                self.thigh_cont.name,
                dummy_hock_cont,
                self.side,
            ...
            # foot roll
            _m_matrix, _, _ = connection.matrixConstraint(
                toe_trans_loc,
                self.hock_ik_cont.get_offsets()[-1],
                source_parent_cutoff=self.localOffGrp,
                maintainOffset=True,
            )
            ...
            cmds.connectAttr(
                "{}.worldMatrix[0]".format(self.localOffGrp),
                "{}.matrixIn[2]".format(_m_matrix),
                force=True,
            )

            # connection.matrixConstraint(toe_trans_loc, self.hock_ik_cont.get_offsets()[-1], maintainOffset=True)
            cmds.connectAttr(
                "%s.footRoll" % self.foot_ik_cont.name, "%s.rx" % toe_trans_loc
            )

    def create_fk_setup(self):
        connection.matrixConstraint(
            self.upper_leg_fk_cont.name,
            self.j_fk_hip,
            maintainOffset=True,
            source_parent_cutoff=self.localOffGrp,
        )
        connection.matrixConstraint(
            self.lower_leg_fk_cont.name,
            self.j_fk_stifle,
            maintainOffset=True,
            source_parent_cutoff=self.localOffGrp,
        )
        connection.matrixConstraint(
            self.pastern_fk_cont.name,
            self.j_fk_hock,
            maintainOffset=True,
            source_parent_cutoff=self.localOffGrp,
        )
        connection.matrixConstraint(
            self.foot_fk_cont.name,
            self.j_fk_phalanges,
            maintainOffset=True,
            source_parent_cutoff=self.localOffGrp,
        )
        ...
        cmds.parent(
            self.lower_leg_fk_cont.get_offsets()[-1], self.upper_leg_fk_cont.name
        )

        attribute.disconnect_attr(
            node=self.j_fk_hip, attr="inverseScale", suppress_warnings=True
        )
        attribute.disconnect_attr(
            node=self.j_fk_stifle, attr="inverseScale", suppress_warnings=True
        )
        attribute.disconnect_attr(
            node=self.j_fk_hock, attr="inverseScale", suppress_warnings=True
        )
        attribute.disconnect_attr(
            node=self.j_fk_phalanges, attr="inverseScale", suppress_warnings=True
        )
    ...
    def ikfk_switching(self):
        ...
        self.upper_leg_fk_cont.drive_visibility(
            "%s.FK_IK_Reverse" % self.switch_cont.name
        )
        self.lower_leg_fk_cont.drive_visibility(
            "%s.FK_IK_Reverse" % self.switch_cont.name
        )
        self.pastern_fk_cont.drive_visibility(
            "%s.FK_IK_Reverse" % self.switch_cont.name
        )
    ...
    def create_ribbons(self):
        ...
        if not self.isLocal:
            cmds.connectAttr(
                "%s.s" % self.scaleHook, "%s.s" % ribbon_upper_leg.scale_grp
            )

        _upper_leg_ore_con = ribbon_upper_leg.orient(
            node_a=self.j_ik_hip,
            node_b=self.j_fk_hip,
        ...
        upper_leg_paired_con = cmds.parentConstraint(
            self.j_def_hindhip,
        ...
        pair_blend_node = cmds.listConnections(
            upper_leg_paired_con, destination=True, type="pairBlend"
        )[0]
        # re-connect to the custom attribute
        cmds.connectAttr(
            "{0}.alignHip".format(self.switch_cont.name),
        ...
        if not self.isLocal:
            cmds.connectAttr(
                "%s.s" % self.scaleHook, "%s.s" % ribbon_lower_leg.scale_grp
            )

        # PASTERN RIBBON
        ribbon_pastern = Ribbon(
            self.j_def_hock,
            self.j_def_phalanges,
        ...
        tweak_conts = (
            ribbon_upper_leg.controllers
            + ribbon_lower_leg.controllers
            + ribbon_pastern.controllers
        )
        attribute.drive_attrs(
            "%s.tweakControls" % self.switch_cont.name,
            ["%s.v" % x.name for x in tweak_conts],
        )

        # add the ribbon deformer joints to the leg module
        self.deformerJoints.extend(
            ribbon_upper_leg.deformer_joints
            + ribbon_lower_leg.deformer_joints
    ...
    def round_up(self):
        ...
        self.anchors = [
            (self.foot_ik_cont.name, "parent", 1, None),
            (self.pole_ik_cont.name, "parent", 1, None),
        ]
...
class Guides(GuidesCore):
    ...
    def draw_joints(self):
        ...
        cmds.select(deselect=True)
        hindleg = cmds.joint(
            position=hindleg_root_vec,
            name=naming.parse([self.name, "root"], side=self.side, suffix="jInit"),
        )
        hip = cmds.joint(
            position=hip_vec,
            name=naming.parse([self.name, "hindHip"], side=self.side, suffix="jInit"),
        )
        stifle = cmds.joint(
            position=stifle_vec,
        ...
        hock = cmds.joint(
            position=hock_vec,
            name=naming.parse([self.name, "hock"], side=self.side, suffix="jInit"),
        )
        toes = cmds.joint(
            position=toes_vec,
        ...
        toetip = cmds.joint(
            position=toetip_vec,
            name=naming.parse(
                [self.name, "phalangesTip"], side=self.side, suffix="jInit"
            ),
        )

...
class Hindleg(object):
    def __init__(self, build_data=None, inits=None, *args, **kwargs):
        ...
        self.suffix = (naming.unique_name(cmds.getAttr("%s.moduleName" % self.hindleg_root_ref)))
    ...
    def create_joints(self):
        ...
        self.limbPlug = cmds.joint(name="jPlug_%s" % self.suffix, position=self.hindleg_root_pos, radius=3)
        ...
        cmds.select(deselect=True)
        self.j_def_hindleg_root = cmds.joint(name="jDef_HindlegRoot_%s" % self.suffix, position=self.hindleg_root_pos,
                                             radius=1.5)
        self.sockets.append(self.j_def_hindleg_root)
        self.deformerJoints.append(self.j_def_hindleg_root)
        self.j_def_hindhip = cmds.joint(name="jDef_Hindhip_%s" % self.suffix, position=self.hindhip_pos, radius=1.5)
        ...
        cmds.select(deselect=True)
        self.j_def_stifle = cmds.joint(name="jDef_Stifle_%s" % self.suffix, position=self.stifle_pos, radius=1.5)
        self.sockets.append(self.j_def_stifle)
        self.deformerJoints.append(self.j_def_stifle)

        cmds.select(deselect=True)
        self.j_def_hock = cmds.joint(name="jDef_Hock_%s" % self.suffix, position=self.hock_pos, radius=1.0)
        self.sockets.append(self.j_def_hock)
        self.deformerJoints.append(self.j_def_hock)

        cmds.select(deselect=True)
        self.j_def_phalanges = cmds.joint(name="jDef_Phalanges_%s" % self.suffix, position=self.phalanges_pos,
                                          radius=1.0)
        self.sockets.append(self.j_def_phalanges)
        self.deformerJoints.append(self.j_def_phalanges)

        cmds.select(deselect=True)
        self.j_phalanges_tip = cmds.joint(name="j_PhalangesTip_%s" % self.suffix, position=self.phalangestip_pos,
                                          radius=1.0)
        ...
        # IK Chain
        cmds.select(deselect=True)
        self.j_ik_hip = cmds.joint(name="jIK_Hindhip_%s" % self.suffix, position=self.hindhip_pos, radius=0.5)
        self.j_ik_stifle = cmds.joint(name="jIK_stifle_%s" % self.suffix, position=self.stifle_pos, radius=0.5)
        self.j_ik_hock = cmds.joint(name="jIK_hock_%s" % self.suffix, position=self.hock_pos, radius=0.5)
        self.j_ik_phalanges = cmds.joint(name="jIK_phalanges_%s" % self.suffix, position=self.phalanges_pos, radius=0.5)
        self.j_ik_phalanges_tip = cmds.joint(name="jIK_phalangesTip_%s" % self.suffix, position=self.phalangestip_pos,
                                             radius=0.5)
        cmds.select(deselect=True)
        ...
        # FK Joints
        cmds.select(deselect=True)
        self.j_fk_hip = cmds.joint(name="jFK_Hindhip_%s" % self.suffix, position=self.hindhip_pos, radius=2.0)
        self.j_fk_stifle = cmds.joint(name="jFK_stifle_%s" % self.suffix, position=self.stifle_pos, radius=2.0)
        self.j_fk_hock = cmds.joint(name="jFK_hock_%s" % self.suffix, position=self.hock_pos, radius=2.0)
        self.j_fk_phalanges = cmds.joint(name="jFK_phalanges_%s" % self.suffix, position=self.phalanges_pos, radius=2.0)
        self.j_fk_phalanges_tip = cmds.joint(name="jFK_phalangesTip_%s" % self.suffix, position=self.phalangestip_pos,
                                             radius=2.0)
    ...
    def create_controllers(self):
        ...
        self.switch_cont = Controller(shape="FkikSwitch", name="%s_FK_IK_cont" % self.suffix, scale=icon_scale)
    ...
    def _create_fk_cont(self, joint, length, name=""):
        ...
        cont = Controller(shape="Cube", name="{0}_{1}_cont".format(self.suffix, name), scale=scale)
    ...
    def create_ik_setup(self):
        # create ik chains
        hock_ik_handle = \
            cmds.ikHandle(startJoint=self.j_ik_hip, endEffector=self.j_ik_hock, name="ikHandle_Hock_%s" % self.suffix,
                          solver="ikRPsolver")[
                0]
        phalanges_ik_handle = \
            cmds.ikHandle(startJoint=self.j_ik_hock, endEffector=self.j_ik_phalanges,
                          name="ikHandle_Phalanges_%s" % self.suffix,
                          solver="ikSCsolver")[0]
        ...
        toe_trans_grp, toe_trans_loc = group_align([hock_ik_grp, phalanges_ik_grp, phalanges_tip_ik_grp],
                                                   self.j_phalanges_tip, "toe_trans")
    ...
    def create_ribbons(self):
        ...
        ribbon_upper_leg = Ribbon(self.j_def_hindhip, self.j_def_stifle, name="up_%s" % self.suffix,
        ...
        # LOWER LEG RIBBON
        ribbon_lower_leg = Ribbon(self.j_def_stifle, self.j_def_hock, name="low_%s" % self.suffix,
                                  connect_start_aim=False,
                                  up_vector=self.up_axis)
...
class Guides(object):
    ...
    def draw_joints(self):
        ...
        cmds.select(deselect=True)
        hindleg = cmds.joint(position=hindleg_root_vec, name="jInit_hindleg_root_%s" % self.suffix)
        hip = cmds.joint(position=hip_vec, name="jInit_hindhip_%s" % self.suffix)
        stifle = cmds.joint(position=stifle_vec, name="jInit_stifle_%s" % self.suffix)
        hock = cmds.joint(position=hock_vec, name="jInit_hock_%s" % self.suffix)
        toes = cmds.joint(position=toes_vec, name="jInit_phalanges_%s" % self.suffix)
        toetip = cmds.joint(position=toetip_vec, name="jInit_phalangestip_%s" % self.suffix)

...
class Eye(_module.ModuleCore):
    ...
    def create_controllers(self):
        ...
        if self.groupID:
            if not self.group_cont:
                ...
                cmds.delete(cmds.pointConstraint([x.name for x in self.other_eye_conts], self.group_cont.name,
                                                 maintainOffset=False))
                group_cont_off = self.group_cont.add_offset("OFF")
                cmds.connectAttr("{}.scale".format(self.scaleGrp), "{}.scale".format(group_cont_off))
                cmds.parent(group_cont_off, self.limbGrp)
            for cont in self.other_eye_conts:
                g_follow = cont.parent
                _ = [attribute.disconnect_attr(g_follow, attr=attr, suppress_warnings=True) for attr in
                ...
                # adjust the pivot
                cmds.xform(self.group_cont.name, absolute=True, worldSpace=True,
                           pivots=api.get_center([x.name for x in self.other_eye_conts]))
                cmds.xform(self.group_cont.parent, absolute=True, worldSpace=True,
                           pivots=api.get_center([x.name for x in self.other_eye_conts]))
    ...
    def create_connections(self):
        _aim_con = cmds.aimConstraint(self.aim_bridge, self.aimDriven, upVector=self.up_axis, aimVector=self.look_axis,
...
class Guides(_module.GuidesCore):
    ...
    def draw_joints(self):
        ...
        root_jnt = cmds.joint(name=naming.parse([self.name, "root"], side=self.side, suffix="jInit"),
                              position=root_point)
        aim_jnt = cmds.joint(name=naming.parse([self.name, "aim"], side=self.side, suffix="jInit"),

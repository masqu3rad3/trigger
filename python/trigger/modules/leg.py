...
class Leg(_module.ModuleCore):
    def __init__(self, build_data=None, inits=None):
        ...
        if build_data:
            ...
            self.inits = [self.leg_root_ref, self.hip_ref, self.knee_ref, self.foot_ref, self.ball_ref,
                          self.heel_pv_ref, self.toe_pv_ref, self.bank_in_ref, self.bank_out_ref]
        elif inits:
            if len(inits) < 9:
        ...
        self.init_foot_length = functions.get_distance(self.toe_pv_ref, self.heel_pv_ref)
        self.init_foot_width = functions.get_distance(self.bank_in_ref, self.bank_out_ref)

        self.up_axis, self.mirror_axis, self.look_axis = joint.get_rig_axes(self.leg_root_ref)
    ...
    def create_joints(self):
        ...
        self.limbPlug = cmds.joint(name=naming.parse([self.module_name, "plug"], suffix="j"),
                                   position=self.leg_root_pos, radius=3)

        self.leg_root_j_def = cmds.joint(name=naming.parse([self.module_name, "legRoot"], suffix="jDef"),
                                         position=self.leg_root_pos, radius=1.5)
        self.sockets.append(self.leg_root_j_def)
        self.hip_j_def = cmds.joint(name=naming.parse([self.module_name, "hip"], suffix="jDef"), position=self.hip_pos,
                                    radius=1.5)
        self.sockets.append(self.hip_j_def)

        if not self.useRefOrientation:
            joint.orient_joints([self.leg_root_j_def, self.hip_j_def], world_up_axis=om.MVector(self.mirror_axis),
                                reverse_aim=self.sideMult)
        else:
            functions.align_to(self.leg_root_j_def, self.leg_root_ref, position=True, rotation=True)
            cmds.makeIdentity(self.leg_root_j_def, apply=True)
            functions.align_to(self.hip_j_def, self.hip_ref, position=True, rotation=True)
            cmds.makeIdentity(self.hip_j_def, apply=True)

        cmds.select(deselect=True)
        self.mid_leg_j_def = cmds.joint(name=naming.parse([self.module_name, "knee"], suffix="jDef"),
                                        position=self.knee_pos, radius=1.5)
        self.sockets.append(self.mid_leg_j_def)

        cmds.select(deselect=True)
        self.foot_j_def = cmds.joint(name=naming.parse([self.module_name, "foot"], suffix="jDef"),
                                     position=self.foot_pos, radius=1.0)
        self.sockets.append(self.foot_j_def)
        self.ball_j_def = cmds.joint(name=naming.parse([self.module_name, "ball"], suffix="jDef"),
                                     position=self.ball_pos, radius=1.0)
        self.sockets.append(self.ball_j_def)
        self.toe_j_def = cmds.joint(name=naming.parse([self.module_name, "toe"], suffix="jDef"),
                                    position=self.toe_pv_pos,
                                    radius=1.0)  # POSSIBLE PROBLEM
        self.sockets.append(self.toe_j_def)

        cmds.select(deselect=True)
        self.socket_ball_j = cmds.joint(name=naming.parse([self.module_name, "ball", "socket"], suffix="j"),
                                        position=self.ball_pos, radius=3)
        self.sockets.append(self.socket_ball_j)
        # IK Joints
        # Follow IK Chain
        cmds.select(deselect=True)
        self.ik_orig_root_j = cmds.joint(name=naming.parse([self.module_name, "IK", "orig", "root"], suffix="j"),
                                         position=self.hip_pos, radius=1.5)
        self.ik_orig_knee_j = cmds.joint(name=naming.parse([self.module_name, "IK", "orig", "knee"], suffix="j"),
                                         position=self.knee_pos, radius=1.5)
        self.ik_orig_end_j = cmds.joint(name=naming.parse([self.module_name, "IK", "orig", "end"], suffix="j"),
                                        position=self.foot_pos, radius=1.5)

        # Single Chain IK
        cmds.select(deselect=True)
        self.ik_sc_root_j = cmds.joint(name=naming.parse([self.module_name, "IK", "sc", "root"], suffix="j"),
                                       position=self.hip_pos, radius=1)
        self.ik_sc_knee_j = cmds.joint(name=naming.parse([self.module_name, "IK", "sc", "knee"], suffix="j"),
                                       position=self.knee_pos, radius=1)
        self.ik_sc_end_j = cmds.joint(name=naming.parse([self.module_name, "IK", "sc", "end"], suffix="j"),
                                      position=self.foot_pos, radius=1)

        # Rotate Plane IK
        cmds.select(deselect=True)
        self.ik_rp_root_j = cmds.joint(name=naming.parse([self.module_name, "IK", "rp", "root"], suffix="j"),
                                       position=self.hip_pos, radius=0.7)
        self.ik_rp_knee_j = cmds.joint(name=naming.parse([self.module_name, "IK", "rp", "knee"], suffix="j"),
                                       position=self.knee_pos, radius=0.7)
        self.ik_rp_end_j = cmds.joint(name=naming.parse([self.module_name, "IK", "rp", "end"], suffix="j"),
                                      position=self.foot_pos, radius=0.7)

        cmds.select(deselect=True)
        self.ik_foot_j = cmds.joint(name=naming.parse([self.module_name, "IK", "foot"], suffix="j"),
                                    position=self.foot_pos, radius=1.0)
        self.ik_ball_j = cmds.joint(name=naming.parse([self.module_name, "IK", "ball"], suffix="j"),
                                    position=self.ball_pos, radius=1.0)
        self.ik_toe_j = cmds.joint(name=naming.parse([self.module_name, "IK", "toe"], suffix="j"),
                                   position=self.toe_pv_pos, radius=1.0)

        cmds.select(deselect=True)

        # orientations
        if not self.useRefOrientation:
            joint.orient_joints([self.ik_orig_root_j, self.ik_orig_knee_j, self.ik_orig_end_j],
                                world_up_axis=om.MVector(self.mirror_axis), reverse_aim=self.sideMult)
        else:
            functions.align_to(self.ik_orig_root_j, self.hip_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_orig_root_j, apply=True)
            functions.align_to(self.ik_orig_knee_j, self.knee_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_orig_knee_j, apply=True)
            functions.align_to(self.ik_orig_end_j, self.foot_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_orig_end_j, apply=True)

        if not self.useRefOrientation:
            joint.orient_joints([self.ik_sc_root_j, self.ik_sc_knee_j, self.ik_sc_end_j],
                                world_up_axis=om.MVector(self.mirror_axis), reverse_aim=self.sideMult)
        else:
            functions.align_to(self.ik_sc_root_j, self.hip_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_sc_root_j, apply=True)
            functions.align_to(self.ik_sc_knee_j, self.knee_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_sc_knee_j, apply=True)
            functions.align_to(self.ik_sc_end_j, self.foot_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_sc_end_j, apply=True)

        if not self.useRefOrientation:
            joint.orient_joints([self.ik_rp_root_j, self.ik_rp_knee_j, self.ik_rp_end_j],
                                world_up_axis=om.MVector(self.mirror_axis), reverse_aim=self.sideMult)
        else:
            functions.align_to(self.ik_rp_root_j, self.hip_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_rp_root_j, apply=True)
            functions.align_to(self.ik_rp_knee_j, self.knee_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_rp_knee_j, apply=True)
            functions.align_to(self.ik_rp_end_j, self.foot_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_rp_end_j, apply=True)

        if not self.useRefOrientation:
            joint.orient_joints([self.ik_foot_j, self.ik_ball_j, self.ik_toe_j],
                                world_up_axis=om.MVector(self.mirror_axis),
                                reverse_aim=self.sideMult)
        else:
            functions.align_to(self.ik_foot_j, self.foot_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_foot_j, apply=True)
            functions.align_to(self.ik_ball_j, self.ball_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_ball_j, apply=True)
            functions.align_to(self.ik_toe_j, self.toe_pv_ref, position=True, rotation=True)
            cmds.makeIdentity(self.ik_toe_j, apply=True)

        # FK Joints
        cmds.select(deselect=True)
        self.fk_root_j = cmds.joint(name=naming.parse([self.module_name, "FK", "upLeg"], suffix="j"),
                                    position=self.hip_pos, radius=1.0)
        self.fk_knee_j = cmds.joint(name=naming.parse([self.module_name, "FK", "knee"], suffix="j"),
                                    position=self.knee_pos, radius=1.0)
        self.fk_foot_j = cmds.joint(name=naming.parse([self.module_name, "FK", "foot"], suffix="j"),
                                    position=self.foot_pos, radius=1.0)
        self.fk_ball_j = cmds.joint(name=naming.parse([self.module_name, "FK", "ball"], suffix="j"),
                                    position=self.ball_pos, radius=1.0)
        self.fk_toe_j = cmds.joint(name=naming.parse([self.module_name, "FK", "toe"], suffix="j"),
                                   position=self.toe_pv_pos, radius=1.0)

        if not self.useRefOrientation:
            joint.orient_joints([self.fk_root_j, self.fk_knee_j, self.fk_foot_j, self.fk_ball_j, self.fk_toe_j],
                                world_up_axis=om.MVector(self.mirror_axis), reverse_aim=self.sideMult)
        else:
            functions.align_to(self.fk_root_j, self.hip_ref, position=True, rotation=True)
            cmds.makeIdentity(self.fk_root_j, apply=True)
            functions.align_to(self.fk_knee_j, self.knee_ref, position=True, rotation=True)
            cmds.makeIdentity(self.fk_knee_j, apply=True)
            functions.align_to(self.fk_foot_j, self.foot_ref, position=True, rotation=True)
            cmds.makeIdentity(self.fk_foot_j, apply=True)
            functions.align_to(self.fk_ball_j, self.ball_ref, position=True, rotation=True)
            cmds.makeIdentity(self.fk_ball_j, apply=True)
            functions.align_to(self.fk_toe_j, self.toe_pv_ref, position=True, rotation=True)
        ...
        self.deformerJoints += [self.mid_leg_j_def, self.hip_j_def, self.leg_root_j_def, self.foot_j_def,
                                self.ball_j_def]
    ...
    def create_controllers(self):
        ...
        cmds.move(0, self.sideMult * (thigh_cont_scale[0] * 2), 0, self.cont_thigh.name, relative=True,
                  objectSpace=True)
        ...
        cmds.addAttr(self.cont_IK_foot.name, shortName="polevector", longName="Pole_Vector", defaultValue=0.0,
                     minValue=0.0, maxValue=1.0,
                     attributeType="double", keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="sUpLeg", longName="Scale_Upper_Leg", defaultValue=1.0,
                     minValue=0.0, attributeType="double", keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="sLowLeg", longName="Scale_Lower_Leg", defaultValue=1.0,
                     minValue=0.0, attributeType="double", keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="squash", longName="Squash", defaultValue=0.0, minValue=0.0,
                     maxValue=1.0, attributeType="double", keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="stretch", longName="Stretch", defaultValue=1.0, minValue=0.0,
                     maxValue=1.0, attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="stretchLimit", longName="StretchLimit", defaultValue=100.0,
                     minValue=0.0,
                     maxValue=1000.0, attributeType="double", keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="softIK", longName="SoftIK", defaultValue=0.0, minValue=0.0,
                     maxValue=100.0, keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="volume", longName="Volume_Preserve", defaultValue=0.0,
                     attributeType="double", keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="bLean", longName="Ball_Lean", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="bRoll", longName="Ball_Roll", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="bSpin", longName="Ball_Spin", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="hRoll", longName="Heel_Roll", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="hSpin", longName="Heel_Spin", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="tRoll", longName="Toes_Roll", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="tSpin", longName="Toes_Spin", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="tWiggle", longName="Toes_Wiggle", defaultValue=0.0,
                     attributeType="double",
                     keyable=True)
        cmds.addAttr(self.cont_IK_foot.name, shortName="bank", longName="Bank", defaultValue=0.0,
                     attributeType="double", keyable=True)

        # Pole Vector Controller
        polecont_scale = ((((self.init_upper_leg_dist + self.init_lower_leg_dist) / 2) / 10),
        ...
        offset_vector_pole = api.get_between_vector(self.mid_leg_j_def, [self.hip_j_def, self.fk_foot_j])

        functions.align_and_aim(self.cont_pole.name,
        ...
        cmds.xform(self.cont_fk_up_leg.name, pivots=(self.sideMult * -(self.init_upper_leg_dist / 2), 0, 0),
                   worldSpace=True)
        ...
        scalecont_fk_low_leg = (
            self.init_lower_leg_dist / 2, self.init_lower_leg_dist / 6, self.init_lower_leg_dist / 6)
        ...
        # move the pivot to the bottom
        cmds.xform(self.cont_fk_low_leg.name, pivots=(self.sideMult * -(self.init_lower_leg_dist / 2), 0, 0),
                   worldSpace=True)
        ...
        functions.align_and_aim(self.cont_fk_ik.name, target_list=[self.fk_foot_j],
                                aim_target_list=[self.mid_leg_j_def],
                                up_vector=self.up_axis, rotate_offset=(self.sideMult * 90, self.sideMult * 90, 0))
        cmds.move(icon_scale * 2, 0, 0, self.cont_fk_ik.name, relative=True, objectSpace=True)
        ...
        cmds.addAttr(self.cont_fk_ik.name, shortName="autoHip", longName="Auto_Hip", defaultValue=1.0,
                     attributeType="float",
                     minValue=0.0, maxValue=1.0, keyable=True)
        cmds.addAttr(self.cont_fk_ik.name, shortName="alignHip", longName="Align_Hip", defaultValue=0.0,
                     attributeType="float",
                     minValue=0.0, maxValue=1.0, keyable=True)
        cmds.addAttr(self.cont_fk_ik.name, shortName="footAutoTwist", longName="Foot_Auto_Twist", defaultValue=1.0,
                     minValue=0.0,
                     maxValue=1.0, attributeType="float", keyable=True)
        cmds.addAttr(self.cont_fk_ik.name, shortName="footManualTwist", longName="Foot_Manual_Twist", defaultValue=0.0,
                     attributeType="float",
                     keyable=True)
        cmds.addAttr(self.cont_fk_ik.name, shortName="upLegAutoTwist", longName="UpLeg_Auto_Twist", defaultValue=1.0,
                     minValue=0.0, maxValue=1.0, attributeType="float", keyable=True)
        cmds.addAttr(self.cont_fk_ik.name, shortName="upLegManualTwist", longName="UpLeg_Manual_Twist",
                     defaultValue=0.0,
                     attributeType="float", keyable=True)
        cmds.addAttr(self.cont_fk_ik.name, shortName="allowScaling", longName="Allow_Scaling", defaultValue=1.0,
                     minValue=0.0,
                     maxValue=1.0, attributeType="float", keyable=True)
        cmds.addAttr(self.cont_fk_ik.name, attributeType="enum", keyable=True, shortName="interpType",
                     longName="Interp_Type",
                     enumName="No_Flip:Average:Shortest:Longest:Cache", defaultValue=0)
        cmds.addAttr(self.cont_fk_ik.name, shortName="tweakControls", longName="Tweak_Controls", defaultValue=0,
                     attributeType="bool")
        cmds.setAttr("%s.tweakControls" % self.cont_fk_ik.name, channelBox=True)
        cmds.addAttr(self.cont_fk_ik.name, shortName="fingerControls", longName="Finger_Controls", defaultValue=1,
                     attributeType="bool")
    ...
    def create_roots(self):
        ...
        _start_lock_weight = cmds.parentConstraint(self.hip_j_def, self.start_lock, skipRotate=["y", "z"],
        ...
        cmds.parentConstraint(self.cont_mid_lock.name, self.mid_lock, maintainOffset=False)
    ...
    def create_ik_setup(self):
        ...
        cmds.aimConstraint(self.toe_pv_ref, foot_plane, worldUpObject=self.foot_ref, worldUpType="object")

        pv_bank_in = cmds.group(name=naming.parse([self.module_name, "piv", "bankIn"], suffix="grp"), empty=True)
        functions.align_to(pv_bank_in, self.bank_in_ref, position=True, rotation=True)
        cmds.makeIdentity(pv_bank_in, apply=True, translate=False, rotate=True, scale=True)
        ...
        cmds.makeIdentity(pv_bank_out, apply=True, translate=False, rotate=True, scale=True)
        cmds.setAttr("%s.rotate" % pv_bank_out, pln_rot[0], pln_rot[1], pln_rot[2])

        pv_toe = cmds.group(name=naming.parse([self.module_name, "piv", "toe"], suffix="grp"), empty=True)
        functions.align_to(pv_toe, self.toe_pv_ref, position=True, rotation=True)
        pv_toe_ore = functions.create_offset_group(pv_toe, "ORE")

        pv_ball = cmds.group(name=naming.parse([self.module_name, "piv", "ball"], suffix="grp"), empty=True)
        ...
        pv_heel = cmds.group(name=naming.parse([self.module_name, "piv", "heel"], suffix="grp"), empty=True)
        ...
        pv_ball_spin = cmds.group(name=naming.parse([self.module_name, "piv", "ballSpin"], suffix="grp"),
                                  empty=True)
        ...
        pv_ball_roll = cmds.group(name=naming.parse([self.module_name, "piv", "ballRoll"], suffix="grp"),
                                  empty=True)
        ...
        pv_ball_lean = cmds.group(name=naming.parse([self.module_name, "piv", "ballLean"], suffix="grp"),
                                  empty=True)
        ...
        ik_handle_sc = cmds.ikHandle(startJoint=self.ik_sc_root_j, endEffector=self.ik_sc_end_j,
                                     name=naming.parse([self.module_name, "sc"], suffix="IKHandle"))
        ik_handle_rp = cmds.ikHandle(startJoint=self.ik_rp_root_j, endEffector=self.ik_rp_end_j,
        ...
        ik_handle_ball = cmds.ikHandle(startJoint=self.ik_foot_j, endEffector=self.ik_ball_j,
                                       name=naming.parse([self.module_name, "ball"], suffix="IKHandle"))
        ik_handle_toe = cmds.ikHandle(startJoint=self.ik_ball_j, endEffector=self.ik_toe_j,
        ...
        stretch_offset = cmds.createNode("plusMinusAverage",
        ...
        ik_stretch_distance_clamp = cmds.createNode("clamp",
                                                    name=naming.parse([self.module_name, "IK", "strecthDistance"],
                                                                      suffix="clamp"))
        ik_stretch_stretchiness_clamp = cmds.createNode("clamp",
                                                        name=naming.parse([self.module_name, "IK", "stretchinessClamp"],
                                                                          suffix="clamp"))
        extra_scale_mult_sc = cmds.createNode("multiplyDivide",
                                              name=naming.parse([self.module_name, "sc", "extraScale"], suffix="mult"))
        initial_divide_sc = cmds.createNode("multiplyDivide",
                                            name=naming.parse([self.module_name, "sc", "initialDivide"], suffix="div"))
        self.initial_length_multip_sc = cmds.createNode("multiplyDivide", name=naming.parse(
            [self.module_name, "sc", "initialLengthMultip"], suffix="mult"))
        stretch_amount_sc = cmds.createNode("multiplyDivide",
                                            name=naming.parse([self.module_name, "sc", "stretchAmount"], suffix="mult"))
        sum_of_j_lengths_sc = cmds.createNode("plusMinusAverage",
                                              name=naming.parse([self.module_name, "sc", "sumOfJLengths"],
                                                                suffix="pma"))
        squashiness_sc = cmds.createNode("blendColors",
                                         name=naming.parse([self.module_name, "sc", "squashiness"], suffix="blend"))
        self.stretchiness_sc = cmds.createNode("blendColors",
                                               name=naming.parse([self.module_name, "sc", "stretchiness"],
                                                                 suffix="blend"))
        ...
        ik_soft_mult2 = cmds.createNode("multDoubleLinear",
                                        name=naming.parse([self.module_name, "ikSoft", "mult2"], suffix="mult"))

        ik_soft_sub3 = cmds.createNode("plusMinusAverage",
        ...
        ik_soft_div2 = cmds.createNode("multiplyDivide",
                                       name=naming.parse([self.module_name, "ikSoft", "div2"], suffix="mult"))
        cmds.setAttr("%s.operation" % ik_soft_div2, 2)

        ik_soft_stretch_amount = cmds.createNode("multiplyDivide",
                                                 name=naming.parse([self.module_name, "ikSoft", "sc", "stretchAmount"],
                                                                   suffix="mult"))
        ...
        cmds.connectAttr("%s.outputR" % self.stretchiness_sc, "%s.input1X" % inverted_str_sc)
        cmds.connectAttr("%s.outputG" % self.stretchiness_sc, "%s.input1Y" % inverted_str_sc)

        cmds.connectAttr("%s.outputX" % inverted_str_sc, "%s.translateX" % self.ik_sc_knee_j)
        cmds.connectAttr("%s.outputY" % inverted_str_sc, "%s.translateX" % self.ik_sc_end_j)
        cmds.connectAttr("%s.outputX" % inverted_str_sc, "%s.translateX" % self.ik_rp_knee_j)
        cmds.connectAttr("%s.outputY" % inverted_str_sc, "%s.translateX" % self.ik_rp_end_j)

        # iksoft related
        cmds.connectAttr("%s.softIK" % self.cont_IK_foot.name, "%s.inputR" % ik_soft_clamp)
        ...
        cmds.connectAttr("%s.rotate" % self.cont_IK_foot.name, "%s.rotate" % self.ik_rp_end_j)
        # Stretch Attributes Controller connections
        cmds.connectAttr("%s.sUpLeg" % self.cont_IK_foot.name, "%s.input2X" % extra_scale_mult_sc)
        cmds.connectAttr("%s.sLowLeg" % self.cont_IK_foot.name, "%s.input2Y" % extra_scale_mult_sc)
        cmds.connectAttr("%s.squash" % self.cont_IK_foot.name, "%s.blender" % squashiness_sc)
        cmds.connectAttr("%s.output1D" % stretch_offset, "%s.maxR" % ik_stretch_distance_clamp)
        cmds.connectAttr("%s.stretch" % self.cont_IK_foot.name, "%s.inputR" % ik_stretch_stretchiness_clamp)
        cmds.connectAttr("%s.stretchLimit" % self.cont_IK_foot.name, "%s.input1D[2]" % stretch_offset)

        #
        # Bind Foot Attributes to the controller
        # create multiply nodes for alignment fix
        mult_al_fix_b_lean = cmds.createNode("multDoubleLinear",
                                             name=naming.parse([self.module_name, "multAlFix", "bLean"], suffix="mult"))
        mult_al_fix_b_roll = cmds.createNode("multDoubleLinear",
                                             name=naming.parse([self.module_name, "multAlFix", "bRoll"], suffix="mult"))
        mult_al_fix_b_spin = cmds.createNode("multDoubleLinear",
                                             name=naming.parse([self.module_name, "multAlFix", "bSpin"], suffix="mult"))
        mult_al_fix_h_roll = cmds.createNode("multDoubleLinear",
                                             name=naming.parse([self.module_name, "multAlFix", "hRoll"], suffix="mult"))
        mult_al_fix_h_spin = cmds.createNode("multDoubleLinear",
                                             name=naming.parse([self.module_name, "multAlFix", "hSpin"], suffix="mult"))
        mult_al_fix_t_roll = cmds.createNode("multDoubleLinear",
                                             name=naming.parse([self.module_name, "multAlFix", "tRoll"], suffix="mult"))
        mult_al_fix_t_spin = cmds.createNode("multDoubleLinear",
                                             name=naming.parse([self.module_name, "multAlFix", "tSpin"], suffix="mult"))
        mult_al_fix_t_wiggle = cmds.createNode("multDoubleLinear",
                                               name=naming.parse([self.module_name, "multAlFix", "tWiggle"],
                                                                 suffix="mult"))
        ...
        cmds.connectAttr("%s.bLean" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_b_lean)
        cmds.connectAttr("%s.bRoll" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_b_roll)
        cmds.connectAttr("%s.bSpin" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_b_spin)
        cmds.connectAttr("%s.hRoll" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_h_roll)
        cmds.connectAttr("%s.hSpin" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_h_spin)
        cmds.connectAttr("%s.tRoll" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_t_roll)
        cmds.connectAttr("%s.tSpin" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_t_spin)
        cmds.connectAttr("%s.tWiggle" % self.cont_IK_foot.name, "%s.input1" % mult_al_fix_t_wiggle)
        ...
        cmds.setDrivenKeyframe("%s.rotateX" % pv_bank_out, currentDriver="%s.bank" % self.cont_IK_foot.name,
                               driverValue=0, value=0,
                               inTangentType='linear', outTangentType='linear')
        cmds.setDrivenKeyframe("%s.rotateX" % pv_bank_out, currentDriver="%s.bank" % self.cont_IK_foot.name,
                               driverValue=90,
                               value=90 * self.sideMult, inTangentType='linear', outTangentType='linear')

        cmds.setDrivenKeyframe("%s.rotateX" % pv_bank_in, currentDriver="%s.bank" % self.cont_IK_foot.name,
                               driverValue=0, value=0,
                               inTangentType='linear', outTangentType='linear')
        cmds.setDrivenKeyframe("%s.rotateX" % pv_bank_in, currentDriver="%s.bank" % self.cont_IK_foot.name,
                               driverValue=-90,
                               value=90 * (-1 * self.sideMult), inTangentType='linear', outTangentType='linear')
        ...
        cmds.parentConstraint(self.cont_IK_foot.name, self.ik_parent_grp, maintainOffset=False)

        # parenting should be after the constraint

        blend_ore_ik_root = cmds.createNode("blendColors",
                                            name=naming.parse([self.module_name, "IK", "ore", "up"], suffix="blend"))

        cmds.connectAttr("%s.rotate" % self.ik_sc_root_j, "%s.color2" % blend_ore_ik_root)
        cmds.connectAttr("%s.rotate" % self.ik_rp_root_j, "%s.color1" % blend_ore_ik_root)
        cmds.connectAttr("%s.output" % blend_ore_ik_root, "%s.rotate" % self.ik_orig_root_j)
        cmds.connectAttr("%s.polevector" % self.cont_IK_foot.name, "%s.blender" % blend_ore_ik_root)

        blend_pos_ik_root = cmds.createNode("blendColors",
                                            name=naming.parse([self.module_name, "IK", "pos", "up"], suffix="blend"))
        cmds.connectAttr("%s.translate" % self.ik_sc_root_j, "%s.color2" % blend_pos_ik_root)
        cmds.connectAttr("%s.translate" % self.ik_rp_root_j, "%s.color1" % blend_pos_ik_root)
        cmds.connectAttr("%s.output" % blend_pos_ik_root, "%s.translate" % self.ik_orig_root_j)
        cmds.connectAttr("%s.polevector" % self.cont_IK_foot.name, "%s.blender" % blend_pos_ik_root)

        blend_ore_ik_knee = cmds.createNode("blendColors",
                                            name=naming.parse([self.module_name, "IK", "ore", "low"], suffix="blend"))
        cmds.connectAttr("%s.rotate" % self.ik_sc_knee_j, "%s.color2" % blend_ore_ik_knee)
        cmds.connectAttr("%s.rotate" % self.ik_rp_knee_j, "%s.color1" % blend_ore_ik_knee)
        ...
        cmds.connectAttr("%s.outputR" % blend_ore_ik_knee, "%s.rotateX" % self.ik_orig_knee_j)
        cmds.connectAttr("%s.outputG" % blend_ore_ik_knee, "%s.rotateY" % self.ik_orig_knee_j)
        cmds.connectAttr("%s.outputB" % blend_ore_ik_knee, "%s.rotateZ" % self.ik_orig_knee_j)
        cmds.connectAttr("%s.polevector" % self.cont_IK_foot.name, "%s.blender" % blend_ore_ik_knee)

        blend_pos_ik_knee = cmds.createNode("blendColors",
                                            name=naming.parse([self.module_name, "IK", "pos", "low"], suffix="blend"))
        cmds.connectAttr("%s.translate" % self.ik_sc_knee_j, "%s.color2" % blend_pos_ik_knee)
        cmds.connectAttr("%s.translate" % self.ik_rp_knee_j, "%s.color1" % blend_pos_ik_knee)
        cmds.connectAttr("%s.output" % blend_pos_ik_knee, "%s.translate" % self.ik_orig_knee_j)
        cmds.connectAttr("%s.polevector" % self.cont_IK_foot.name, "%s.blender" % blend_pos_ik_knee)

        blend_ore_ik_end = cmds.createNode("blendColors",
                                           name=naming.parse([self.module_name, "IK", "ore", "lowEnd"], suffix="blend"))
        ...
        # if there is no skincluster, it works ok, but adding a skin cluster misses 2 out of 3 rotation axis
        # Therefore make SEPERATE CONNECTIONS
        cmds.connectAttr("%s.outputR" % blend_ore_ik_end, "%s.rotateX" % self.ik_orig_end_j)
        cmds.connectAttr("%s.outputG" % blend_ore_ik_end, "%s.rotateY" % self.ik_orig_end_j)
        cmds.connectAttr("%s.outputB" % blend_ore_ik_end, "%s.rotateZ" % self.ik_orig_end_j)
        cmds.connectAttr("%s.polevector" % self.cont_IK_foot.name, "%s.blender" % blend_ore_ik_end)

        blend_pos_ik_end = cmds.createNode("blendColors",
                                           name=naming.parse([self.module_name, "IK", "pos", "lowEnd"], suffix="blend"))
        cmds.connectAttr("%s.translate" % self.ik_sc_end_j, "%s.color2" % blend_pos_ik_end)
        cmds.connectAttr("%s.translate" % self.ik_rp_end_j, "%s.color1" % blend_pos_ik_end)
        cmds.connectAttr("%s.output" % blend_pos_ik_end, "%s.translate" % self.ik_orig_end_j)
        cmds.connectAttr("%s.polevector" % self.cont_IK_foot.name, "%s.blender" % blend_pos_ik_end)

        pole_vector_rvs = cmds.createNode("reverse",
        ...
        functions.align_to(pacon_locator_hip, self.leg_root_j_def, position=True, rotation=True)
        #
        _pa_con_j_def = cmds.parentConstraint(self.cont_thigh.name, pacon_locator_hip, maintainOffset=False)
        #
        ...
        cmds.connectAttr("%s.rigVis" % self.scaleGrp, "%s.v" % functions.get_shapes(pacon_locator_hip)[0])

        cmds.delete(foot_plane)

    def create_fk_setup(self):
        cmds.connectAttr("%s.scaleX" % self.cont_fk_up_leg.name, "%s.scaleX" % self.fk_root_j)
        cmds.connectAttr("%s.scaleX" % self.cont_fk_low_leg.name, "%s.scaleX" % self.fk_knee_j)
        cmds.connectAttr("%s.scaleX" % self.cont_fk_foot.name, "%s.scaleX" % self.fk_foot_j)
        cmds.connectAttr("%s.scaleX" % self.cont_fk_ball.name, "%s.scaleX" % self.fk_ball_j)

        cmds.orientConstraint(self.cont_fk_up_leg.name, self.fk_root_j, maintainOffset=False)
        cmds.pointConstraint(self.start_lock, self.fk_root_j, maintainOffset=False)

        cmds.orientConstraint(self.cont_fk_low_leg.name, self.fk_knee_j, maintainOffset=False)
        cmds.orientConstraint(self.cont_fk_foot.name, self.fk_foot_j, maintainOffset=False)

        cmds.parentConstraint(self.cont_fk_ball.name, self.fk_ball_j, maintainOffset=False)

        # TODO : TAKE A LOOK TO THE OFFSET SOLUTION
        cmds.parentConstraint(self.cont_thigh.name, self.cont_fk_up_leg_off, skipRotate=["x", "y", "z"],
                              maintainOffset=True)
    ...
    def ik_fk_switching(self):
        ...
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s.visibility" % self.cont_fk_low_leg_ore)
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s.visibility" % self.cont_fk_foot_ore)
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s.visibility" % self.cont_fk_ball_ore)
        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.visibility" % self.cont_IK_foot.name)
        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.visibility" % self.cont_pole_vis)

        mid_lock_pa_con_weight = \
            cmds.parentConstraint(self.ik_orig_root_j, self.fk_root_j, self.cont_mid_lock_pos, maintainOffset=False)[0]
        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.%sW0" % (mid_lock_pa_con_weight, self.ik_orig_root_j))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name,
        ...
        mid_lock_po_con_weight = \
            cmds.pointConstraint(self.ik_orig_knee_j, self.fk_knee_j, self.cont_mid_lock_ave, maintainOffset=False)[0]
        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.%sW0" % (mid_lock_po_con_weight, self.ik_orig_knee_j))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name,
                         "%s.%sW1" % (mid_lock_po_con_weight, self.fk_knee_j))

        mid_lock_x_bln = cmds.createNode("multiplyDivide",
                                         name=naming.parse([self.module_name, "midLock", "xBln"], suffix="loc"))

        mid_lock_rot_xsw = cmds.createNode("blendTwoAttr",
                                           name=naming.parse([self.module_name, "midLock", "rotXsw"], suffix="blend"))
        cmds.connectAttr("%s.rotateZ" % self.ik_orig_knee_j, "%s.input[0]" % mid_lock_rot_xsw)
        cmds.connectAttr("%s.rotateZ" % self.fk_knee_j, "%s.input[1]" % mid_lock_rot_xsw)
        ...
        end_lock_weight = \
            cmds.pointConstraint(self.ik_orig_end_j, self.fk_foot_j, self.end_lock_pos, maintainOffset=False)[0]
        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.%sW0" % (end_lock_weight, self.ik_orig_end_j))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s.%sW1" % (end_lock_weight, self.fk_foot_j))
        ...
        # ######
        end_lock_rot = \
            cmds.parentConstraint(self.ik_parent_grp, self.fk_foot_j, self.end_lock, skipTranslate=["x", "y", "z"],
                                  maintainOffset=True)[0]

        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.%sW0" % (end_lock_rot, self.ik_parent_grp))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s.%sW1" % (end_lock_rot, self.fk_foot_j))
        cmds.connectAttr("%s.interpType" % self.cont_fk_ik.name, "%s.interpType" % end_lock_rot)

        foot_pa_con = cmds.parentConstraint(self.ik_foot_j, self.fk_foot_j, self.foot_j_def, maintainOffset=False)[0]
        ball_pa_con = cmds.parentConstraint(self.ik_ball_j, self.fk_ball_j, self.ball_j_def, maintainOffset=False)[0]
        toe_pa_con = cmds.parentConstraint(self.ik_toe_j, self.fk_toe_j, self.toe_j_def, maintainOffset=False)[0]

        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.%sW0" % (foot_pa_con, self.ik_foot_j))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s.%sW1" % (foot_pa_con, self.fk_foot_j))
        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s." % "%s.%sW0" % (ball_pa_con, self.ik_ball_j))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s." % "%s.%sW1" % (ball_pa_con, self.fk_ball_j))
        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s." % "%s.%sW0" % (toe_pa_con, self.ik_toe_j))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s." % "%s.%sW1" % (toe_pa_con, self.fk_toe_j))
        cmds.connectAttr("%s.interpType" % self.cont_fk_ik.name, "%s.interpType" % foot_pa_con)
        cmds.connectAttr("%s.interpType" % self.cont_fk_ik.name, "%s.interpType" % ball_pa_con)
        cmds.connectAttr("%s.interpType" % self.cont_fk_ik.name, "%s.interpType" % toe_pa_con)

    def create_def_joints(self):
        # UPPERLEG RIBBON

        ribbon_upper_leg = Ribbon(self.hip_j_def, self.mid_leg_j_def, name=naming.parse([self.module_name, "up"]),
                                  connect_start_aim=False, up_vector=self.mirror_axis)
        ...
        cmds.connectAttr("%s.scale" % self.cont_mid_lock.name, "%s.scale" % ribbon_upper_leg.end_plug)
        cmds.connectAttr("%s.scale" % self.cont_mid_lock.name, "%s.scale" % self.mid_leg_j_def)

        cmds.scaleConstraint(self.scaleGrp, ribbon_upper_leg.scale_grp)

        ribbon_start_ori_con = \
            cmds.parentConstraint(self.ik_orig_root_j, self.fk_root_j, ribbon_upper_leg.start_aim, maintainOffset=False,
                                  skipTranslate=["x", "y", "z"])[0]
        _ribbon_start_ori_con_ext = \
            cmds.parentConstraint(self.hip_j_def, ribbon_upper_leg.start_aim, maintainOffset=False,
                                  skipTranslate=["x", "y", "z"])[0]

        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.%sW0" % (ribbon_start_ori_con, self.ik_orig_root_j))
        cmds.connectAttr("%s.FK_IK_Reverse" % self.cont_fk_ik.name, "%s.%sW1" % (ribbon_start_ori_con, self.fk_root_j))
        ...
        cmds.connectAttr("%s.alignHip" % self.cont_fk_ik.name, "%s.w" % pair_blend_node, force=True)

        # Rotate the shoulder connection bone 180 degrees for Right Alignment
        if self.side == "R":
            right_rbn_startup_ore = cmds.listRelatives(ribbon_upper_leg.start_aim, children=True, type="transform")[0]
            cmds.setAttr("%s.ry" % right_rbn_startup_ore, 180)

        # AUTO AND MANUAL TWIST

        # auto
        auto_twist_thigh = cmds.createNode("multiplyDivide",
        ...
        # manual
        add_manual_twist_thigh = cmds.createNode("plusMinusAverage",
                                                 name=naming.parse([self.module_name, "upperLeg", "manualTwist"],
                                                                   suffix="pma"))
        cmds.connectAttr("%s.output" % auto_twist_thigh, "%s.input3D[0]" % add_manual_twist_thigh)
        cmds.connectAttr("%s.upLegManualTwist" % self.cont_fk_ik.name,
                         "%s.input3D[1].input3Dx" % add_manual_twist_thigh)

        # connect to the joint
        cmds.connectAttr("%s.output3D" % add_manual_twist_thigh, "%s.rotate" % ribbon_upper_leg.start_plug)

        # connect allowScaling
        cmds.connectAttr("%s.allowScaling" % self.cont_fk_ik.name, "%s.scaleSwitch" % ribbon_upper_leg.start_plug)

        # LOWERLEG RIBBON

        ribbon_lower_leg = Ribbon(self.mid_leg_j_def, self.foot_j_def, name=naming.parse([self.module_name, "low"]),
        ...
        cmds.connectAttr("%s.scale" % self.cont_mid_lock.name, "%s.scale" % ribbon_lower_leg.start_plug)
        ...
        # auto
        auto_twist_ankle = cmds.createNode("multiplyDivide",
        ...
        # !!! The parent constrain override should be disconnected like this
        cmds.disconnectAttr("%s.constraintRotateX" % ribbon_start_pa_con_lower_leg_end,
                            "%s.rotateX" % ribbon_lower_leg.end_plug)

        # manual
        add_manual_twist_ankle = cmds.createNode("plusMinusAverage",
                                                 name=naming.parse([self.module_name, "lowerLeg", "manualTwist"],
                                                                   suffix="pma"))
        ...
        # connect to the joint
        cmds.connectAttr("%s.output3D" % add_manual_twist_ankle, "%s.rotate" % ribbon_lower_leg.end_plug)

        # connect allowScaling
        cmds.connectAttr("%s.allowScaling" % self.cont_fk_ik.name, "%s.scaleSwitch" % ribbon_lower_leg.start_plug)

        # Volume Preservation Stuff
        vp_extra_input = cmds.createNode("multiplyDivide",
                                         name=naming.parse([self.module_name, "vpExtraInput"], suffix="mult"))
        cmds.setAttr("%s.operation" % vp_extra_input, 1)

        vp_mid_average = cmds.createNode("plusMinusAverage",
                                         name=naming.parse([self.module_name, "vpMidAverage"], suffix="pma"))
        cmds.setAttr("%s.operation" % vp_mid_average, 3)

        vp_power_mid = cmds.createNode("multiplyDivide",
                                       name=naming.parse([self.module_name, "vpPowerMid"], suffix="mult"))
        cmds.setAttr("%s.operation" % vp_power_mid, 3)
        vp_init_length = cmds.createNode("multiplyDivide",
                                         name=naming.parse([self.module_name, "vpInitLength"], suffix="mult"))
        cmds.setAttr("%s.operation" % vp_init_length, 2)

        vp_power_upper_leg = cmds.createNode("multiplyDivide",
                                             name=naming.parse([self.module_name, "vpPowerUpperLeg"], suffix="mult"))
        cmds.setAttr("%s.operation" % vp_power_upper_leg, 3)

        vp_power_lower_leg = cmds.createNode("multiplyDivide",
                                             name=naming.parse([self.module_name, "vpPowerLowerLeg"], suffix="mult"))
        cmds.setAttr("%s.operation" % vp_power_lower_leg, 3)

        vp_upper_lower_reduce = cmds.createNode("multDoubleLinear",
                                                name=naming.parse([self.module_name, "vpUpperLowerReduce"],
                                                                  suffix="mult"))
        cmds.setAttr("%s.input2" % vp_upper_lower_reduce, 0.5)

        # vp knee branch
        cmds.connectAttr("%s.output" % vp_extra_input, "%s.scale" % ribbon_lower_leg.start_plug, force=True)
        cmds.connectAttr("%s.output" % vp_extra_input, "%s.scale" % ribbon_upper_leg.end_plug, force=True)
        cmds.connectAttr("%s.output" % vp_extra_input, "%s.scale" % self.mid_leg_j_def, force=True)
        cmds.connectAttr("%s.scale" % self.cont_mid_lock.name, "%s.input1" % vp_extra_input, force=True)
        cmds.connectAttr("%s.output1D" % vp_mid_average, "%s.input2X" % vp_extra_input, force=True)
        cmds.connectAttr("%s.output1D" % vp_mid_average, "%s.input2Y" % vp_extra_input, force=True)
        cmds.connectAttr("%s.output1D" % vp_mid_average, "%s.input2Z" % vp_extra_input, force=True)
        cmds.connectAttr("%s.outputX" % vp_power_mid, "%s.input1D[0]" % vp_mid_average, force=True)
        cmds.connectAttr("%s.outputY" % vp_power_mid, "%s.input1D[1]" % vp_mid_average, force=True)
        cmds.connectAttr("%s.outputX" % vp_init_length, "%s.input1X" % vp_power_mid, force=True)
        cmds.connectAttr("%s.outputY" % vp_init_length, "%s.input1Y" % vp_power_mid, force=True)
        cmds.connectAttr("%s.volume" % self.cont_IK_foot.name, "%s.input2X" % vp_power_mid, force=True)
        cmds.connectAttr("%s.volume" % self.cont_IK_foot.name, "%s.input2Y" % vp_power_mid, force=True)
        cmds.connectAttr("%s.outputX" % self.initial_length_multip_sc, "%s.input1X" % vp_init_length, force=True)
        cmds.connectAttr("%s.outputY" % self.initial_length_multip_sc, "%s.input1Y" % vp_init_length, force=True)
        cmds.connectAttr("%s.color1R" % self.stretchiness_sc, "%s.input2X" % vp_init_length, force=True)
        cmds.connectAttr("%s.color1G" % self.stretchiness_sc, "%s.input2Y" % vp_init_length, force=True)
        ...
        cmds.connectAttr("%s.outputX" % vp_init_length, "%s.input1X" % vp_power_upper_leg)
        cmds.connectAttr("%s.output" % vp_upper_lower_reduce, "%s.input2X" % vp_power_upper_leg)
        ...
        cmds.connectAttr("%s.outputX" % vp_init_length, "%s.input1X" % vp_power_lower_leg)
        cmds.connectAttr("%s.output" % vp_upper_lower_reduce, "%s.input2X" % vp_power_lower_leg)
        cmds.connectAttr("%s.volume" % self.cont_IK_foot.name, "%s.input1" % vp_upper_lower_reduce)
        ...
        self.deformerJoints += ribbon_lower_leg.deformer_joints + ribbon_upper_leg.deformer_joints
    ...
    def create_angle_extractors(self):
        # IK Angle Extractor
        angle_ext_root_ik = \
            cmds.spaceLocator(name=naming.parse([self.module_name, "IK", "angleExt", "root"], suffix="loc"))[0]
        angle_ext_fixed_ik = \
            cmds.spaceLocator(name=naming.parse([self.module_name, "IK", "angleExt", "fixed"], suffix="loc"))[0]
        angle_ext_float_ik = \
            cmds.spaceLocator(name=naming.parse([self.module_name, "IK", "angleExt", "float"], suffix="loc"))[0]
        ...
        cmds.parentConstraint(self.cont_IK_foot.name, angle_ext_fixed_ik, maintainOffset=False)
        ...
        angle_node_ik = cmds.createNode("angleBetween", name=naming.parse([self.module_name, "IK"], suffix="angle"))
        angle_remap_ik = cmds.createNode("remapValue",
                                         name=naming.parse([self.module_name, "IK", "angle"], suffix="remap"))
        angle_mult_ik = cmds.createNode("multDoubleLinear",
                                        name=naming.parse([self.module_name, "IK", "angle"], suffix="mult"))

        cmds.connectAttr("%s.translate" % angle_ext_fixed_ik, "%s.vector1" % angle_node_ik)
        cmds.connectAttr("%s.translate" % angle_ext_float_ik, "%s.vector2" % angle_node_ik)
        cmds.connectAttr("%s.angle" % angle_node_ik, "%s.inputValue" % angle_remap_ik)

        cmds.setAttr("%s.inputMin" % angle_remap_ik, cmds.getAttr("%s.angle" % angle_node_ik))
        cmds.setAttr("%s.inputMax" % angle_remap_ik, 0)
        cmds.setAttr("%s.outputMin" % angle_remap_ik, 0)
        cmds.setAttr("%s.outputMax" % angle_remap_ik, cmds.getAttr("%s.angle" % angle_node_ik))
        ...
        # FK Angle Extractor
        angle_remap_fk = cmds.createNode("remapValue",
                                         name=naming.parse([self.module_name, "FK", "angle"], suffix="remap"))
        angle_mult_fk = cmds.createNode("multDoubleLinear",
                                        name=naming.parse([self.module_name, "FK", "angle"], suffix="mult"))

        cmds.connectAttr("%s.rotateZ" % self.cont_fk_up_leg.name, "%s.inputValue" % angle_remap_fk)
        ...
        angle_ext_blend = cmds.createNode("blendTwoAttr",
                                          name=naming.parse([self.module_name, "angleExt"], suffix="blend"))
        angle_global = cmds.createNode("multDoubleLinear",
                                       name=naming.parse([self.module_name, "angleGlobal"], suffix="mult"))

        cmds.connectAttr("%s.fk_ik" % self.cont_fk_ik.name, "%s.attributesBlender" % angle_ext_blend)
        ...
        cmds.connectAttr("%s.autoHip" % self.cont_fk_ik.name, "%s.input2" % angle_global)
        cmds.connectAttr("%s.output" % angle_global, "%s.rotateZ" % self.cont_thigh_auto)
...
class Guides(_module.GuidesCore):
    ...
    def draw_joints(self):
        ...
        root = cmds.joint(position=root_vec, name=naming.parse([self.name, "legRoot"], side=self.side, suffix="jInit"))
        hip = cmds.joint(position=hip_vec, name=naming.parse([self.name, "hip"], side=self.side, suffix="jInit"))
        knee = cmds.joint(position=knee_vec, name=naming.parse([self.name, "knee"], side=self.side, suffix="jInit"))
        foot = cmds.joint(position=foot_vec, name=naming.parse([self.name, "foot"], side=self.side, suffix="jInit"))
        joint.orient_joints([root, hip, knee, foot], world_up_axis=self.mirrorVector, up_axis=(0, 1, 0),
                            reverse_aim=self.sideMultiplier)

        ball = cmds.joint(position=ball_vec, name=naming.parse([self.name, "ball"], side=self.side, suffix="jInit"))
        toe = cmds.joint(position=toe_vec, name=naming.parse([self.name, "toe"], side=self.side, suffix="jInit"))
        cmds.select(clear=True)
        bankout = cmds.joint(position=bankout_vec,
                             name=naming.parse([self.name, "bankOut"], side=self.side, suffix="jInit"))
        cmds.select(clear=True)
        bankin = cmds.joint(position=bankin_vec,
                            name=naming.parse([self.name, "bankIn"], side=self.side, suffix="jInit"))
        cmds.select(clear=True)
        toepv = cmds.joint(position=toepv_vec,
                           name=naming.parse([self.name, "toePivot"], side=self.side, suffix="jInit"))
        cmds.select(clear=True)
        heelpv = cmds.joint(position=heelpv_vec,
        ...
        joint.orient_joints([ball, toe], world_up_axis=self.mirrorVector, up_axis=(0, 1, 0),
                            reverse_aim=self.sideMultiplier)

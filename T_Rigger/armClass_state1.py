...
class Arm(object):
    ...
    def initialization(self):
        ...
        master_root = pm.group(em=True, name="masterRoot_%s" % self.suffix)
        extra.alignTo(master_root, self.collar_ref, 0)
        pm.makeIdentity(master_root, a=True)

        master_ik = pm.spaceLocator(name="masterIK_%s" % self.suffix)
        extra.alignTo(master_ik, self.hand_ref, 0)

        collar_pos = self.collar_ref.getTranslation(space="world")
        shoulder_pos = self.shoulder_ref.getTranslation(space="world")
        elbow_pos = self.elbow_ref.getTranslation(space="world")
        hand_pos = self.hand_ref.getTranslation(space="world")
        init_shoulder_dist = extra.getDistance(self.collar_ref, self.shoulder_ref)
        init_upper_arm_dist = extra.getDistance(self.shoulder_ref, self.elbow_ref)
        init_lower_arm_dist = extra.getDistance(self.elbow_ref, self.hand_ref)

        root_position = self.hand_ref.getTranslation(space="world")
    ...
    def createarm(self, arminits, suffix="", side="L"):
        ...
        pm.setAttr("{0}.s{1}".format(cont_shoulder_pos, "z"), sideMult)

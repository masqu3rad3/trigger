...
class Arm(object):
    ...
    def createJoints(self):
        ...
        # orientations
        extra.orientJoints([self.j_ik_orig_up, self.j_ik_orig_low, self.j_ik_orig_low_end], localMoveAxis=(dt.Vector(self.up_axis)),
                           mirrorAxis=(self.sideMult, 0.0, 0.0), upAxis=self.sideMult*(dt.Vector(self.up_axis)))

        extra.orientJoints([self.j_ik_sc_up, self.j_ik_sc_low, self.j_ik_sc_low_end], localMoveAxis=(dt.Vector(self.up_axis)),
                           mirrorAxis=(self.sideMult, 0.0, 0.0), upAxis=self.sideMult*(dt.Vector(self.up_axis)))

        extra.orientJoints([self.j_ik_rp_up, self.j_ik_rp_low, self.j_ik_rp_low_end], localMoveAxis=(dt.Vector(self.up_axis)),
                           mirrorAxis=(self.sideMult, 0.0, 0.0), upAxis=self.sideMult*(dt.Vector(self.up_axis)))
        ...
        extra.orientJoints([self.j_fk_up, self.j_fk_low, self.j_fk_low_end], localMoveAxis=(dt.Vector(self.up_axis)), mirrorAxis=(self.sideMult, 0.0, 0.0), upAxis=self.sideMult*(dt.Vector(self.up_axis)))
    ...
    def createControllers(self):
        ...
        extra.alignAndAim(self.cont_shoulder, targetList=[self.j_def_collar], aimTargetList=[self.j_collar_end], upVector=self.up_axis)
        ...
        extra.alignAndAim(self.cont_fk_ik, targetList=[self.j_def_hand], aimTargetList=[self.j_def_elbow], upVector=self.up_axis, rotateOff=(0, 180, 0))
        ...
        pm.addAttr(self.cont_fk_ik, shortName="handAutoTwist", longName="Hand_Auto_Twist", defaultValue=1.0, minValue=0.0,
                   maxValue=1.0, at="float", k=True)
        pm.addAttr(self.cont_fk_ik, shortName="handManualTwist", longName="Hand_Manual_Twist", defaultValue=0.0, at="float",
                   k=True)
    ...
    def createIKsetup(self):
        ...
        ik_handle_rp = pm.ikHandle(sj=self.j_ik_rp_up, ee=self.j_ik_rp_low_end, name="ikHandle_RP_%s" % self.suffix, sol="ikRPsolver")
    ...
    def ikfkSwitching(self):
        ...
        mid_lock_pa_con_weight = pm.orientConstraint(self.j_ik_orig_low, self.j_fk_low, self.cont_mid_lock_pos, mo=False)
        ...
        end_lock_rot = pm.parentConstraint(self.ik_parent_grp, self.cont_fk_hand, self.end_lock_twist, st=("x", "y", "z"), mo=False)
        ...
        hand_ori_con = pm.parentConstraint(self.cont_IK_hand, self.hand_lock, self.root_master, st=("x", "y", "z"), mo=False)
    ...
    def createDefJoints(self):
        ...
        ribbon_upper_arm.createPowerRibbon(self.j_collar_end, self.j_def_elbow, "up_%s" % self.suffix, side=self.side, orientation=0, connectStartAim=False, upVector=self.up_axis)

        ribbon_start_pa_con_upper_arm_start = pm.parentConstraint(self.start_lock, ribbon_upper_arm.startConnection, mo=False)
        ...
        ribbon_start_ori_con = pm.parentConstraint(self.j_ik_orig_up, self.j_fk_up, ribbon_upper_arm.startAim, mo=False, skipTranslate=["x", "y", "z"])

        ribbon_start_ori_con2 = pm.parentConstraint(self.j_collar_end, ribbon_upper_arm.startAim, mo=False, skipTranslate=["x", "y", "z"])
        ...
        ribbon_lower_arm.createPowerRibbon(self.j_def_elbow, self.j_def_hand, "low_%s" % self.suffix, side=self.side, orientation=0, upVector=self.up_axis)

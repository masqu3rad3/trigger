...
scaleGrp=pm.group(name="scaleGrp_splineIK", em=True)
nonScaleGrp=pm.group(name="nonScaleGrp_splineIK", em=True)
...
for i in range (0, len(defJoints)):
    ...
    curveGlobMult=pm.createNode("multiplyDivide", name="curveGlobMult")
    pm.setAttr(curveGlobMult.operation,2)
    boneGlobMult=pm.createNode("multiplyDivide", name="boneGlobMult")
    
    lengthMult=pm.createNode("multiplyDivide", name="length_Multiplier")
    pm.setAttr(lengthMult.operation, 2)
    
    volumeSw=pm.createNode("blendColors", name="volumeSw")
    stretchSw=pm.createNode("blendTwoAttr", name="stretchSw")
    
    middlePoint=(len(defJoints)/2)
    volumePow=pm.createNode("multiplyDivide", name="volume_Power")
    volumeFactor=pm.createNode("multiplyDivide", name="volume_Factor")
